...
if ($w == "")
{
    ...
    $sql = " select MAX(SUBSTRING(ca_id,$len2,2)) as max_subid from {$site_fix}.{$g5['g5_shop_category_table']}
...
$frm_submit = '<div class="btn_confirm01 btn_confirm">
    ...
    <a href="./categorylist.php?'.$qstr.'&site_code='.$site_code.'">목록</a>
</div>';

// 쿠폰 적용 불가 설정 필드 추가
if(!sql_query(" select ca_nocoupon from {$site_fix}.{$g5['g5_shop_category_table']} limit 1 ", false)) {
    sql_query(" ALTER TABLE `{$site_fix}.{$g5['g5_shop_category_table']}`
                    ADD `ca_nocoupon` tinyint(4) NOT NULL DEFAULT '0' AFTER `ca_adult_use` ", true);
}

// 스킨 디렉토리 필드 추가
if(!sql_query(" select ca_skin_dir from {$site_fix}.{$g5['g5_shop_category_table']} limit 1 ", false)) {
    sql_query(" ALTER TABLE `{$site_fix}.{$g5['g5_shop_category_table']}`
...
// 분류 출력순서 필드 추가
if(!sql_query(" select ca_order from {$site_fix}.{$g5['g5_shop_category_table']} limit 1 ", false)) {
    sql_query(" ALTER TABLE `{$site_fix}.{$g5['g5_shop_category_table']}`
                    ADD `ca_order` int(11) NOT NULL DEFAULT '0' AFTER `ca_name` ", true);
    sql_query(" ALTER TABLE `{$site_fix}.{$g5['g5_shop_category_table']}` ADD INDEX(`ca_order`) ", true);
}

// 모바일 상품 출력줄수 필드 추가
if(!sql_query(" select ca_mobile_list_row from {$site_fix}.{$g5['g5_shop_category_table']} limit 1 ", false)) {
    sql_query(" ALTER TABLE `{$site_fix}.{$g5['g5_shop_category_table']}`
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <?php } else { ?>
                ...
                <!-- <a href="<?php echo G5_SHOP_URL; ?>/list.php?ca_id=<?php echo $ca_id; ?>&site_code=<?=$site_code?>" class="btn_frmline">미리보기</a> -->
                <a href="./categoryform.php?ca_id=<?php echo $ca_id; ?>&amp;<?php echo $qstr; ?>&site_code=<?=$site_code?>" class="btn_frmline">하위분류 추가</a>
                <a href="./itemlist.php?sca=<?php echo $ca['ca_id']; ?>&site_code=<?=$site_code?>" class="btn_frmline">상품리스트</a>
            <?php } ?>
            </td>
        </tr>
        <tr>
