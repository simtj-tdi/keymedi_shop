...
if ($w == "")
{
    ...
    $sql = " insert {$site_fix}.{$g5['g5_shop_category_table']}
                set ca_id   = '$ca_id',
                    ca_name = '$ca_name',
                    $sql_common ";
    sql_query($sql);
...
{
    $sql = " update {$site_fix}.{$g5['g5_shop_category_table']}
                set ca_name = '$ca_name',
                    $sql_common
              where ca_id = '$ca_id' ";
    sql_query($sql);
...
else if ($w == "d")
{
    ...
    $sql = " select COUNT(*) as cnt from {$site_fix}.{$g5['g5_shop_category_table']}
              where SUBSTRING(ca_id,1,$len) = '$ca_id'
                and ca_id <> '$ca_id' ";
    $row = sql_fetch($sql);
    if ($row['cnt'] > 0)
    ...
    $str = $comma = "";
    $sql = " select it_id from {$site_fix}.{$g5['g5_shop_item_table']} where ca_id = '$ca_id' ";
    ...
    // 분류 삭제
    $sql = " delete from {$site_fix}.{$g5['g5_shop_category_table']} where ca_id = '$ca_id' ";
    sql_query($sql);
}

if ($w == "" || $w == "u")
{
    goto_url("./categoryform.php?w=u&amp;ca_id=$ca_id&amp;site_code=$site_code&amp;$qstr");
} else {
