...
if($ca_use == "1" || $ca_use == "0"){
	$sql_search .= " and  ca_use = '$ca_use' ";
}
...
$sql_common = " from {$site_fix}.{$g5['g5_shop_category_table']} ";
if ($is_admin != 'super')
...
<select name="ca_use">
	...
	<option value="1" <?php echo get_selected($_GET['ca_use'], "1", true); ?>>사용</option>
	<option value="0" <?php echo get_selected($_GET['ca_use'], "0", true); ?>>미사용</option>
</select>
...
<div class="btn_add01 btn_add">
    <a href="./categoryform.php?site_code=<?=$site_fix?>" id="cate_add">분류 추가</a>
	<? if($is_admin) { ?>
		<a href="./category_down.php?site_code=<?=$site_fix?>">엑셀다운로드</a>
	<? } ?>
</div>
...
<div id="sct" class="tbl_head02 tbl_wrap">
    ...
    <thead>
    <tr>
		<!-- <th scope="col" rowspan="2"><input type="checkbox" name="chkall" value="1" id="chkall" onclick="check_all2(this.form)"> 사이트구분</th> -->
        <th scope="col" rowspan="2" width="4%"><?php echo subject_sort_link("ca_id"); ?>분류코드</a></th>
        <th scope="col" id="sct_cate"><?php echo subject_sort_link("ca_name"); ?>분류명</a></th>
        <th scope="col" id="sct_amount" width="5%">상품수</th>
        <th scope="col" id="sct_imgw" width="5%">이미지 폭</th>
        <th scope="col" id="sct_imgh" width="5%">이미지 높이</th>
        <th scope="col" id="sct_imgcol" width="5%">1행이미지수</th>
        <th scope="col" id="sct_mobileimg" width="5%">모바일 1행이미지수</th>
        <th scope="col" id="sct_pcskin"width="12%">PC스킨지정</th>
        <th scope="col" rowspan="2"width="8%">관리</th>
    </tr>
    <tr>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        if ($level > 0) {
            ...
            $sql = " select ca_name from {$site_fix}.{$g5['g5_shop_category_table']} where ca_id = '$p_ca_id' ";
            $temp = sql_fetch($sql);
        ...
        if ($level+2 < 6) $s_add = '<a href="./categoryform.php?ca_id='.$row['ca_id'].'&amp;site_code='.$site_fix.'&amp;'.$qstr.'" class="btn btn-info btn-xs">추가</a> '; // 분류는 5단계까지만 가능
        else $s_add = '';
        $s_upd = '<a href="./categoryform.php?w=u&amp;ca_id='.$row['ca_id'].'&amp;site_code='.$site_fix.'&amp;'.$qstr.'" class="btn btn-primary btn-xs"><span class="sound_only">'.get_text($row['ca_name']).' </span>수정</a> ';

        if ($is_admin == 'super')
            $s_del = '<a href="./categoryformupdate.php?w=d&amp;ca_id='.$row['ca_id'].'&amp;site_code='.$site_fix.'&amp;'.$qstr.'" onclick="return delete_confirm(this);" class="btn btn-danger btn-xs"><span class="sound_only">'.get_text($row['ca_name']).' </span>삭제</a> ';

        // 해당 분류에 속한 상품의 수
        $sql1 = " select COUNT(*) as cnt from {$site_fix}.{$g5['g5_shop_item_table']}
                      where ( ca_id = '{$row['ca_id']}'
                      or ca_id2 = '{$row['ca_id']}'
                      or ca_id3 = '{$row['ca_id']}' ) and it_8 = '' ";
        $row1 = sql_fetch($sql1);
		...
		//shop_juny
		$array_code = explode(",", $row['site_code']);
		$array_site = "";
		foreach($array_code as $val) { 
			$array_site .= "'".$val."',"; 
		} 
		$array_site = substr($array_site, 0, -1);
		$site_name_tmp = "";
		$site_name = "";
		$site_sql = sql_query("select code_name from site_code where code in ( ".$array_site." ) ");
		for($j = 0 ; $site_name_tmp = sql_fetch_array($site_sql);$j++){
			$site_name .= $site_name_tmp[code_name]."<BR>";
		}

    ?>
    <tr class="<?php echo $bg; ?>">
		<!-- <td style="width:150px;line-height:2em;" rowspan="2">
		<?
			$site_sql = sql_query("select code , code_name from site_code ");
			for($j = 0 ; $site_name_tmp = sql_fetch_array($site_sql);$j++){
				$pos = strpos($row['site_code'], $site_name_tmp[code]);
				$in_ck = false;
				if($pos === false) {
					$in_ck = false;
				} else {
			...
			?>
			<input type="checkbox" class="chkbx" name="items[<?php echo $i; ?>][<?php echo $j; ?>]" value="<?=$site_name_tmp[code]?>" <?=($in_ck)?"checked":""?> /> <?=$site_name_tmp[code_name]?> <br>
			<?
			}
		?>
		</td> -->
...
<!-- //shop_juny -->
<?php echo get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, "{$_SERVER['SCRIPT_NAME']}?$qstr&amp;site_code=".$site_code."&amp;page="); ?>
