...
if($_FILES['excelfile']['tmp_name']) {
    ...
    */
    ...
    for ($i = 3; $i <= $data->sheets[0]['numRows']; $i++) {
		...
		$it_3				= addslashes($data->sheets[0]['cells'][$i][$j++]);
		$it_4				= addslashes($data->sheets[0]['cells'][$i][$j++]);
		$it_5				= addslashes($data->sheets[0]['cells'][$i][$j++]);
		$it_6				= addslashes($data->sheets[0]['cells'][$i][$j++]);
		$it_8				= addslashes($data->sheets[0]['cells'][$i][$j++]);
		$it_9				= addslashes($data->sheets[0]['cells'][$i][$j++]);
		if($is_admin){
			$it_10				= addslashes($data->sheets[0]['cells'][$i][$j++]);
		}else{
			addslashes($data->sheets[0]['cells'][$i][$j++]);
			$it_10	= $member[mb_id];
		}
        ...
        // it_id 중복체크
        $sql2 = " select count(*) as cnt from {$site_fix}.{$g5['g5_shop_item_table']} where it_id = '$it_id' ";
        $row2 = sql_fetch($sql2);
        if($row2['cnt']) {
            $fail_it_id[] = $it_id;
            $dup_it_id[] = $it_id;
        ...
        // 기본분류체크
        $sql2 = " select count(*) as cnt from {$site_fix}.{$g5['g5_shop_category_table']} where ca_id = '$ca_id' ";
        $row2 = sql_fetch($sql2);
        if(!$row2['cnt']) {
            $fail_it_id[] = $it_id;
            $fail_count++;
        ...
        $sql = " INSERT INTO {$site_fix}.{$g5['g5_shop_item_table']}
                     SET it_id = '$it_id',
                         ca_id = '$ca_id',
                         ca_id2 = '$ca_id2',
                         ca_id3 = '$ca_id3',
