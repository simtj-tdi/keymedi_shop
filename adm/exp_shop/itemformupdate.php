...
define('G5_DATA_PATH2',"/data/was/".$site_fix."/data");
 

@mkdir(G5_DATA_PATH2."/item", G5_DIR_PERMISSION);
@chmod(G5_DATA_PATH2."/item", G5_DIR_PERMISSION);
...
if($w == "u") {
    $sql = " select it_img1, it_img2, it_img3, it_img4, it_img5, it_img6, it_img7, it_img8, it_img9, it_img10
                from {$site_fix}.{$g5['g5_shop_item_table']}
                where it_id = '$it_id' ";
    $file = sql_fetch($sql);
...
if ($_FILES['it_img1']['name']) {
	...
	$it_img1 = str_replace($it_img_dir,"",$it_img1);
}
if ($_FILES['it_img2']['name']) {
	...
	$it_img2 = str_replace($it_img_dir,"",$it_img2);
}
if ($_FILES['it_img3']['name']) {
	...
	$it_img3 = str_replace($it_img_dir,"",$it_img3);
}
if ($_FILES['it_img4']['name']) {
	...
	$it_img4 = str_replace($it_img_dir,"",$it_img4);
}
if ($_FILES['it_img5']['name']) {
	...
	$it_img5 = str_replace($it_img_dir,"",$it_img5);
}
if ($_FILES['it_img6']['name']) {
	...
	$it_img6 = str_replace($it_img_dir,"",$it_img6);
}
if ($_FILES['it_img7']['name']) {
	...
	$it_img7 = str_replace($it_img_dir,"",$it_img7);
}
if ($_FILES['it_img8']['name']) {
	...
	$it_img8 = str_replace($it_img_dir,"",$it_img8);
}
if ($_FILES['it_img9']['name']) {
	...
	$it_img9 = str_replace($it_img_dir,"",$it_img9);
}
if ($_FILES['it_img10']['name']) {
	...
	$it_img10 = str_replace($it_img_dir,"",$it_img10);
}
...
// 관련상품을 우선 삭제함
sql_query(" delete from {$site_fix}.{$g5['g5_shop_item_relation_table']} where it_id = '$it_id' ");

// 관련상품의 반대도 삭제
sql_query(" delete from {$site_fix}.{$g5['g5_shop_item_relation_table']} where it_id2 = '$it_id' ");

// 이벤트상품을 우선 삭제함
sql_query(" delete from {$site_fix}.{$g5['g5_shop_event_item_table']} where it_id = '$it_id' ");

// 선택옵션
sql_query(" delete from {$site_fix}.{$g5['g5_shop_item_option_table']} where io_type = '0' and it_id = '$it_id' "); // 기존선택옵션삭제
...
// 추가옵션
sql_query(" delete from {$site_fix}.{$g5['g5_shop_item_option_table']} where io_type = '1' and it_id = '$it_id' "); // 기존추가옵션삭제
...
$sql_common = " ca_id               = '$ca_id',
                ...
                it_8                = '$it_8',
                 it_9                = '$it_9' ,
				 it_s_commission                = '$it_s_commission' ,
				it_commission                = '$it_commission' ,
				it_tax              = '$it_tax' ";
if(!$is_admin){
	$sql_common .= " , it_10               = '$member[mb_id]' ";
}
...
if ($w == "" || $w == "u")
{
    ...
    for ($i=0; $i<count($it_id2); $i++)
    {
        if (trim($it_id2[$i]))
        {
            $sql = " insert into {$site_fix}.{$g5['g5_shop_item_relation_table']}
                        set it_id  = '$it_id',
                            it_id2 = '$it_id2[$i]',
                            ir_no = '$i' ";
            sql_query($sql, false);
    ...
    for ($i=0; $i<count($ev_id); $i++)
    {
        if (trim($ev_id[$i]))
        {
            $sql = " insert into {$site_fix}.{$g5['g5_shop_event_item_table']}
                        set ev_id = '$ev_id[$i]',
                            it_id = '$it_id' ";
            sql_query($sql, false);
        }
...
if($option_count) {
    $comma = '';
    $sql = " INSERT INTO {$site_fix}.{$g5['g5_shop_item_option_table']}
...
if($ca_fields) {
    sql_query(" update {$site_fix}.{$g5['g5_shop_item_table']} set it_name = it_name {$ca_fields} where ca_id = '$ca_id' ");
    if($ca_id2)
        sql_query(" update {$site_fix}.{$g5['g5_shop_item_table']} set it_name = it_name {$ca_fields} where ca_id2 = '$ca_id2' ");
    if($ca_id3)
        sql_query(" update {$site_fix}.{$g5['g5_shop_item_table']} set it_name = it_name {$ca_fields} where ca_id3 = '$ca_id3' ");
}
...
if($all_fields) {
    sql_query(" update {$site_fix}.{$g5['g5_shop_item_table']} set it_name = it_name {$all_fields} ");
}

$qstr = "$qstr&amp;sca=$sca&amp;page=$page";

if ($w == "u") {
    goto_url("./itemform.php?w=u&amp;it_id=$it_id&amp;$qstr&amp;site_code=$site_fix");
} else if ($w == "d")  {
...
<script>
    if (confirm("계속 입력하시겠습니까?"))
        ...
        location.href = "<?php echo "./itemform.php?".str_replace('&amp;', '&', $qstr); ?>&site_code=<?=$site_fix?>";
    else
        location.href = "<?php echo "./itemlist.php?".str_replace('&amp;', '&', $qstr); ?>&site_code=<?=$site_fix?>";
</script>
