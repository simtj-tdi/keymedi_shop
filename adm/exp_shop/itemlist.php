...
*/
$sql_common = " from {$g5['g5_shop_item_table']} a   where ( 1 = 1 ";
/*
if($s_mas =="my"){
	if ($is_admin != 'super')
...
<? if($is_admin) { ?>
	<a href="./itemexcel_down.php?site_code=<?=$site_fix?>">엑셀다운로드</a>
<? } ?>
    <a href="./itemform.php">마스터상품등록</a>
    <a href="./itemexcel.php?site_code=<?=$site_fix?>" onclick="return excelform(this.href);" target="_blank">상품일괄등록</a>
</div>
...
<div class="tbl_head02 tbl_wrap">
    ...
    <tr class="<?php echo $bg; ?>">
		...
		<?
			...
			$msql = "select count(DISTINCT(it_10)) as cnt from {$site_fix}.{$g5['g5_shop_item_table']} where it_8 = '$row[it_id]' ";
			$mem = sql_fetch($msql);
			echo $mem[cnt];

			}else{
				$dis_cnt = sql_fetch(" select count(*) as cnt from {$site_fix}.{$g5['g5_shop_item_table']} where it_8 = '$row[it_id]' and it_10 = '$member[mb_id]' ");
				if($dis_cnt[cnt] > 0 ){
					echo "Y";
				}else{
					echo "N";
		...
		<?
			$min = sql_fetch("select min(it_price) as price from {$site_fix}.{$g5['g5_shop_item_table']} where it_8 = '{$row['it_id']}' and it_use = '1' and it_soldout = '0' ");
		?>
        ...
        <td rowspan="3" class="td_mng">
			...
			<? } ?>
			<? if($is_admin) { ?>
            <a href="./itemform.php?w=u&amp;it_id=<?php echo $row['it_id']; ?>&amp;ca_id=<?php echo $row['ca_id']; ?>&amp;<?php echo $qstr; ?>" class="btn btn-primary btn-xs"><span class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'],250, "")); ?> </span>수정</a>
            <? } ?>
            <!-- <a href="<?php echo $href; ?>" target="_blank"><span class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'],250, "")); ?> </span>보기</a> -->
        </td>
    </tr>
    <tr class="<?php echo $bg; ?>">
        <td rowspan="2" class="td_img"><a href="<?php echo $href; ?>"><?php echo get_it_image2($site_fix,"/data/was/".$site_fix."/data","http://skin.keymedi.com",$row['it_id'], 50, 50); ?></a></td>
