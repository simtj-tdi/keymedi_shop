...
if ($stx != "") {
    if($sfl == "it_8" || $sfl == "it_9") {
        ...
        $sql_search .= " $where (select $sfl from g5_shop_item where it_id = a.it_8) like '%$stx%' ";
        $where = " and ";
    }
    if ($save_stx != $stx)
        $page = 1;
...
<p>
<label for="site_code">사이트구분 : </label>
<select name="site_code" id="site_code">
	<option value="">::사이트구분::</option>
	<?
		$sql_site = "select * from site_code where exp = 'yes' ";
		$res_site = sql_query($sql_site);
		while($row_site = sql_fetch_array($res_site)){
	?>
		<option value="<?=$row_site[code]?>" <?=($row_site[code]==$site_fix)?"selected":"" ?>><?=$row_site[code_name]?></option>
	<? } ?>
</select>
</p>
<p>
...
<div class="btn_add01 btn_add">
 <? if($is_admin) { ?>
	<a href="./itemexcel_down2.php?m_class=<?=$m_class?>&site_code=<?=$site_fix?>">엑셀다운로드</a>
<? } ?>
	<a href="./itemexcel_up.php?site_code=<?=$site_fix?>" onclick="return excelform(this.href);" target="_blank">상품일괄수정</a>
	 
   <a href="./itemexcel2.php?site_code=<?=$site_fix?>" onclick="return excelform(this.href);" target="_blank">상품일괄등록</a>
</div>
...
<div class="tbl_head02 tbl_wrap">
    ...
    <tr>
		...
		<?if($is_admin){?>
		<th scope="col" rowspan="3" width="8%">셀러상품코드</th>
		<? } ?>
		<th scope="col" rowspan="3" width="5%">업체코드</th>
		...
		<th scope="col" rowspan="3" width="5%">규격</th>
		<th scope="col" rowspan="3" width="3%">단위</th>
		<th scope="col" rowspan="3" width="8%">제조사</th>
        ...
        <th scope="col" rowspan="3" width="3%">관리</th>
    </tr>
    <tr>
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td <? if($is_admin){?>colspan="5"<?}else{?>colspan="4"<?}?>>
            ...
            <select name="ca_id[<?php echo $i; ?>]" id="ca_id_<?php echo $i; ?>" style="width:220px;">
            ...
            <select name="ca_id2[<?php echo $i; ?>]" id="ca_id2_<?php echo $i; ?>"  style="width:220px;">
            ...
            <select name="ca_id3[<?php echo $i; ?>]" id="ca_id3_<?php echo $i; ?>"  style="width:220px;">
		...
		<?
			$min = sql_fetch("select min(it_price) as price from {$site_fix}.{$g5['g5_shop_item_table']} where it_8 = '{$row['it_8']}' and it_use = '1' and it_soldout = '0' ");
		?>
        ...
        <td rowspan="3" class="td_mng">
			<? if($row['it_8']==""){ ?>
			<a href="./itemcopy.php?it_id=<?php echo $row['it_id']; ?>&amp;ca_id=<?php echo $row['ca_id']; ?>&site_code=<?=$site_fix?>" class="itemcopy btn btn-info btn-xs" target="_blank"><span class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'],250, "")); ?> </span>상품등록</a>
			<? } ?>
			<? if($is_admin/* || $row['it_10'] == $member[mb_id]*/) { ?>
            <a href="./itemform.php?w=u&amp;it_id=<?php echo $row['it_id']; ?>&amp;ca_id=<?php echo $row['ca_id']; ?>&amp;<?php echo $qstr; ?>&site_code=<?=$site_fix?>" class="btn btn-primary btn-xs"><span class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'],250, "")); ?> </span>수정</a>
            <? } ?>
            <!-- <a href="<?php echo $href; ?>" target="_blank"><span class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'],250, "")); ?> </span>보기</a> -->
        </td>
    </tr>
    <tr class="<?php echo $bg; ?>">
        <td rowspan="2" class="td_img"><a href="<?php echo $href; ?>"><?php echo get_it_image2($site_fix,"/data/was/".$site_fix."/data","http://skin.keymedi.com",$row['it_8'], 50, 50); ?></a></td>
