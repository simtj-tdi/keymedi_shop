...
$it_id_ar = "";
$sqld = sql_query("select it_id from {$site_fix}.{$g5['g5_shop_item_table']} {$sql_search} group by it_8");
while($wr = sql_fetch_array($sqld)){
	if($it_id_ar == ""){
		$it_id_ar = $wr['it_id'];
	}else{
		$it_id_ar = $it_id_ar.",".$wr['it_id'];
	}
}
...
$sql_common = "  from {$site_fix}.{$g5['g5_shop_item_table']} ";
//$sql_common .= $sql_search;
if ($stx != "") {
$sql_common .= " where it_id in (".$it_id_ar.") ";
}
// 테이블의 전체 레코드수만 얻음
...
<select name="sca" id="sca">
   ...
   // $sql1 = " select ca_id, ca_name from {$g5['g5_shop_category_table']} order by ca_order, ca_id ";
	 $sql1 = " select ca_id, ca_name from {$g5['g5_shop_category_table']} order by ca_name asc";
...
<input type="hidden" name="page" value="<?php echo $page; ?>">
<input type="hidden" name="site_code" value="<?php echo $site_fix; ?>">
...
<div class="tbl_head01 tbl_wrap">
    ...
    <thead>
    <tr>
        <th scope="col" width="8%"><?php echo subject_sort_link("it_id", $qstr, 1); ?>마스터코드</a></th>
        <th scope="col"><?php echo subject_sort_link("it_name"); ?>상품명</a></th>
		<th scope="col" width="3%"><?php echo subject_sort_link("it_type3", $qstr, 1); ?>신규상품</a></th>
        <th scope="col" width="3%"><?php echo subject_sort_link("it_type2", $qstr, 1); ?>추천상품</a></th>
        <th scope="col" width="3%"><?php echo subject_sort_link("it_type1", $qstr, 1); ?>베스트상품</a></th>
        <!-- <th scope="col"><?php echo subject_sort_link("it_type4", $qstr, 1); ?>인기<br>상품</a></th> -->
        <th scope="col" width="3%"><?php echo subject_sort_link("it_type5", $qstr, 1); ?>특가상품</a></th>
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td class="td_chk">
		 ...
		 <td class="td_chk">
            <label for="type1_<?php echo $i; ?>" class="sound_only">베스트상품</label>
        ...
        <td class="td_chk">
            <label for="type5_<?php echo $i; ?>" class="sound_only">특가상품</label>
            <input type="checkbox" name="it_type5[<?php echo $i; ?>]" value="1" id="type5_<?php echo $i; ?>" <?php echo ($row['it_type5'] ? 'checked' : ''); ?>>
        </td>
        <td class="td_mngsmall">
            <a href="./itemform.php?w=u&amp;it_id=<?php echo $row['it_id']; ?>&amp;ca_id=<?php echo $row['ca_id']; ?>&amp;<?php echo $qstr; ?>&amp;site_code=<?=$site_fix?>" class="btn btn-primary btn-xs"><span class="sound_only"><?php echo cut_str(stripslashes($row['it_name']), 60, "&#133"); ?> </span>수정</a>
         </td>
    </tr>
    <?php
    }
