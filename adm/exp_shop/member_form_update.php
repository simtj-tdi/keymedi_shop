...
if ($w == '')
{
    ...
    // 닉네임중복체크
    $sql = " select mb_id, mb_name, mb_nick, mb_email from {$site_fix}.g5_member where mb_nick = '{$_POST['mb_nick']}' ";
    $row = sql_fetch($sql);
    if ($row['mb_id'])
        alert('이미 존재하는 닉네임입니다.\\nＩＤ : '.$row['mb_id'].'\\n이름 : '.$row['mb_name'].'\\n닉네임 : '.$row['mb_nick'].'\\n메일 : '.$row['mb_email']);

    // 이메일중복체크
    $sql = " select mb_id, mb_name, mb_nick, mb_email from {$site_fix}.g5_member where mb_email = '{$_POST['mb_email']}' ";
    $row = sql_fetch($sql);
    if ($row['mb_id'])
        alert('이미 존재하는 이메일입니다.\\nＩＤ : '.$row['mb_id'].'\\n이름 : '.$row['mb_name'].'\\n닉네임 : '.$row['mb_nick'].'\\n메일 : '.$row['mb_email']);

    sql_query(" insert into {$site_fix}.g5_member set mb_id = '{$mb_id}', mb_password = '".get_encrypt_string($mb_password)."', mb_datetime = '".G5_TIME_YMDHIS."', mb_ip = '{$_SERVER['REMOTE_ADDR']}', mb_email_certify = '".G5_TIME_YMDHIS."', {$sql_common} ");
...
if ($w == ''){
	alert('등록되었습니다.','./'.$return_url.'?'.$qstr.'&amp;w=u&amp;site_code='.$site_fix.'&amp;mb_id='.$mb_id, false);
}else if ($w == 'u'){
	alert('수정되었습니다.','./'.$return_url.'?'.$qstr.'&amp;w=u&amp;site_code='.$site_fix.'&amp;mb_id='.$mb_id, false);
}
