...
$sql_common = " from {$site_fix}.g5_member ";
...
<div class="btn_add01 btn_add">
    ...
    <a href="./member_form.php?site_code=<?=$site_fix?>" id="member_add">회원추가</a>
</div>
<?php } ?>
...
<? } ?>
    ...
    <tr>
		...
		<th scope="col" id="mb_list_point"><?php echo subject_sort_link('mb_point', '', 'desc') ?> 포인트</th>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if ($is_admin == 'group') {
            ...
            $s_mod = '<a href="./member_form.php?'.$qstr.'&amp;w=u&amp;mb_id='.$row['mb_id'].'&amp;site_code='.$site_fix.'" class="btn btn-primary btn-xs">수정</a>';
        }
    ...
    <tr class="<?php echo $bg; ?>">
		...
		<td headers="mb_list_id" rowspan="2" class="td_name sv_use"><?=($row[mb_where]=="메디포털")?"키메디":$row[mb_where] ?></td>
		...
		if($row[mb_where] == "메디포털"){
			...
			$icon_file = '/data/was/portal/data/member/'.$mb_dir.'/'.$row['mb_id']."_saup";
			$icon_file2 = '/data/was/portal/data/member/'.$mb_dir.'/'.$row['mb_id']."_saup.pdf";
			if (file_exists($icon_file) || file_exists($icon_file2)) {
				if(file_exists($icon_file)){
					echo "<a href='http://www.keymedi.com/data/member/{$mb_dir}/{$row[mb_id]}_saup' target='_blank'>O</a>";
				}
				if(file_exists($icon_file2)){
					echo "<a href='http://www.keymedi.com/data/member/{$mb_dir}/{$row[mb_id]}_saup.pdf' target='_blank'>O</a>";
				}
			}else{
				echo "X";
			}
			...
			$mb_dir = substr($row['mb_id'],0,2);
			$icon_file = '/data/was/union/data/member/'.$mb_dir.'/'.$row['mb_id'];
			if (file_exists($icon_file)) {
				echo "<a href='http://obgy.keymedi.com/data/member/{$mb_dir}/{$row[mb_id]}' target='_blank'>O</a>";		
			}else{
				echo "X";
			}
		}else if($row[mb_where] == "피부비만"){
			$mb_dir = substr($row['mb_id'],0,2);
			$icon_file = '/data/was/shop/data/member/'.$mb_dir.'/'.$row['mb_id'];
			if (file_exists($icon_file)) {
				echo "<a href='http://obgys.keymedi.com/data/member/{$mb_dir}/{$row[mb_id]}' target='_blank'>O</a>";		
			}else{
				echo "X";
			}	
		}
