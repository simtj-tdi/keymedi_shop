...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
		...
		<th scope="col" width="8%">위치</th>
        <th scope="col">제목</th>
        <th scope="col" width="3%">FAQ수</th>
        <th scope="col" width="3%">순서</th>
        <th scope="col" width="8%">관리</th>
    </tr>
    </thead>
    <tbody>
	<?
		function ck_input($a , $b){
			$ae = explode(",", $a);	
			$be = explode(",", $b);
			$ret = false;
			foreach($be as $val) { 
    ...
    <tr class="<?php echo $bg; ?>">
		...
		<td style="width:150px;">
			...
			<?
				$sql_site = "select * from site_code ";
				$res_site = sql_query($sql_site);
				while($row_site = sql_fetch_array($res_site)){
			?>
				<?=(ck_input($row_site[code],$row['site_code']))?$row_site[code_name]."<br>":""?>
			<? } ?> 
		</td>
        ...
        <td class="td_mng">
            <a href="./faqmasterform.php?w=u&amp;fm_id=<?php echo $row['fm_id']; ?>" class="btn btn-primary btn-xs"><span class="sound_only"><?php echo stripslashes($row['fm_subject']); ?> </span>수정</a>
            <a href="<?php echo G5_BBS_URL; ?>/faq.php?fm_id=<?php echo $row['fm_id']; ?>" class="btn btn-dark btn-xs"><span class="sound_only"><?php echo stripslashes($row['fm_subject']); ?> </span>보기</a>
            <a href="./faqmasterformupdate.php?w=d&amp;fm_id=<?php echo $row['fm_id']; ?>" onclick="return delete_confirm(this);" class="btn btn-danger btn-xs"><span class="sound_only"><?php echo stripslashes($row['fm_subject']); ?> </span>삭제</a>
        </td>
    </tr>
    <?php
    }
