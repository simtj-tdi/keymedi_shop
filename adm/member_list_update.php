...
if ($_POST['act_button'] == "선택수정") {
    ...
    for ($i=0; $i<count($_POST['chk']); $i++)
    {
        // 실제 번호를 넘김
        $k = $_POST['chk'][$i];
		$mb_where = $_POST['mb_where'][$k];
		if($_POST['mb_where'][$k] == "메디포털"){
			$mb = sql_fetch("select * from portal.g5_member where mb_id = '{$_POST['mb_id'][$k]}' ");
...
goto_url('./member_list.php?'.$qstr."&mb_where=".$mb_where);
?>
