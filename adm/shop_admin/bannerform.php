...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <td>
            ...
            <select name="bn_position" id="bn_position" onchange="view_px(this.value);return false;">
				...
				<!--<option value="피부비만왼쪽" <?php /*echo get_selected($bn['bn_position'], '피부비만왼쪽'); */?>>피부비만왼쪽</option>
                <option value="피부비만메인" <?php /*echo get_selected($bn['bn_position'], '피부비만메인'); */?>>피부비만메인</option>-->
				...
				<option value="산협몰_상단" <?php echo get_selected($bn['bn_position'], '산협몰_상단'); ?>>산부인과몰 상단</option>

                <option value="베스트상품_상단부" <?php echo get_selected($bn['bn_position'], '베스트상품_상단부'); ?>>베스트상품_상단부</option>

        </select>

            <span id="view_pxs" style="color:red;">( WIDTH : 900px HEIGHT : 490px )</span>
            <script>
                function view_px(val) {
                    var view = document.getElementById("view_pxs");
                    if (val == "메인") {
                        view.innerHTML = "( WIDTH : 900px HEIGHT : 490px )";
                    } else if (val == "키메디메인") {
                        view.innerHTML = "( WIDTH : 900px HEIGHT : 490px )";
                    } else if (val == "공통메인_이벤트") {
                        if(<?=$bn['bn_device']?>=='pc'){
                            view.innerHTML = "( WIDTH : 980px HEIGHT : 548px )";
                        }else if(<?=$bn['bn_device']?>=='mobile'){
                            view.innerHTML = "( WIDTH : 590px HEIGHT : 330px )";
                        }else{
                            view.innerHTML = "( WIDTH : 980px HEIGHT : 548px )";
                        }


                    } else if (val == "산협몰 로그인페이지") {
                        view.innerHTML = "( WIDTH : 1060px HEIGHT : 150px )";
                    } else if (val == "키메디몰_상단") {
                        view.innerHTML = "( WIDTH : 1180px HEIGHT : 120px )";
                    } else if (val == "산협몰_상단") {
                        view.innerHTML = "( WIDTH : 1180px HEIGHT : 120px )";
                    } else if (val == "베스트상품_상단부") {
                        view.innerHTML = "( WIDTH : 1180px HEIGHT : 제한없음 )";
                    }
                }

                view_px("<?= $bn['bn_position'] ?>");
            </script>
        </td>
    </tr>
    <tr>
