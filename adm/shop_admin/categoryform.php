...
<input type="hidden" name="site_code" value="<?=$ca['site_code']?>">
<section id="anc_scatefrm_basic">
    ...
    <div class="tbl_frm01 tbl_wrap">
		...
		<!-- //shop_juny -->
		<tr>
            <th scope="row"><label for="ca_id">사이트코드</label></th>
            <td> 
			...
			?>
				<input type="checkbox" name="items[]" value="<?=$row_site[code]?>" <?=(ck_input($row_site[code],$ca['site_code']))?"checked":""?>/> <?=$row_site[code_name]?>&nbsp;&nbsp;&nbsp;&nbsp; 
			<? } ?> 
			</td>
		</th>
        <tr>
...
function fcategoryformcheck(f)
{
	...
	if (!bchk)
	{
		alert("사이트코드를 하나 이상 선택하세요.");
		return false;
	}
	f.site_code.value = btxt;
