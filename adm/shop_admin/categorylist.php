...
//shop_juny
if($site_code){
	$sql_search .= " and  site_code like '%$site_code%' ";
}
...
<select name="site_code" id="site_code">
	...
	<?
		$sql_site = "select * from site_code where exp = 'no' ";
		$res_site = sql_query($sql_site);
		while($row_site = sql_fetch_array($res_site)){
	?>
		<option value="<?=$row_site[code]?>" <?=($row_site[code]==$site_code)?"selected":"" ?>><?=$row_site[code_name]?></option>
	<? } ?>
</select>
...
<div class="btn_add01 btn_add">
	...
	<? if($is_admin) { ?>
		<a href="./category_down.php">엑셀다운로드</a>
	<? } ?>
</div>
...
<div id="sct" class="tbl_head02 tbl_wrap">
    ...
    <thead>
    <tr>
		<th scope="col" rowspan="2" width="10%"><input type="checkbox" name="chkall" value="1" id="chkall" onclick="check_all2(this.form)"> 사이트구분</th>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        if ($level+2 < 6) $s_add = '<a href="./categoryform.php?ca_id='.$row['ca_id'].'&amp;'.$qstr.'" class="btn btn-info btn-xs">추가</a> '; // 분류는 5단계까지만 가능
        else $s_add = '';
        $s_upd = '<a href="./categoryform.php?w=u&amp;ca_id='.$row['ca_id'].'&amp;'.$qstr.'" class="btn btn-primary btn-xs"><span class="sound_only">'.get_text($row['ca_name']).' </span>수정</a> ';

        if ($is_admin == 'super')
            $s_del = '<a href="./categoryformupdate.php?w=d&amp;ca_id='.$row['ca_id'].'&amp;'.$qstr.'" onclick="return delete_confirm(this);" class="btn btn-danger btn-xs"><span class="sound_only">'.get_text($row['ca_name']).' </span>삭제</a> ';
    ...
    <tr class="<?php echo $bg; ?>">
		...
		<?
			$site_sql = sql_query("select code , code_name from site_code where exp = 'no' ");
			for($j = 0 ; $site_name_tmp = sql_fetch_array($site_sql);$j++){
