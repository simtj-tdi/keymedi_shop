...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <td>
            <input type="text" name="mb_id" value="<?php echo stripslashes($cp['mb_id']); ?>" id="mb_id" class="frm_input" onclick="clear_input();return false;">
            <button type="button" id="sch_member" class="btn_frmline">회원검색</button>
			<input type="radio" name="chk_all_mb" id="chk_all_mb" value="1" <?=($cp[mb_id]=="전체회원")?"checked":""?>><label for="chk_all_mb"> 전체회원</label>&nbsp;&nbsp;
            <input type="radio" name="chk_all_mb" id="chk_all_mb1" value="2" <?=($cp[mb_id]=="산부인과 협동조합")?"checked":""?>><label for="chk_all_mb1"> 산부인과몰</label>&nbsp;&nbsp;
			<input type="radio" name="chk_all_mb" id="chk_all_mb2" value="3" <?=($cp[mb_id]=="메디포털")?"checked":""?>><label for="chk_all_mb2"> 키메디</label>&nbsp;&nbsp;
			<input type="radio" name="chk_all_mb" id="chk_all_mb3" value="4" <?=($cp[mb_id]=="피부비만")?"checked":""?>><label for="chk_all_mb2"> 피부비만</label>
...
function clear_input(){ 
	 var chk = document.getElementsByName("chk_all_mb");
	var bchk = false;
	var btxt = "";
...
function form_check(f)
{
	...
	*/
	if(!f.chk_all_mb.value && f.mb_id.value == "") {
        alert("회원아이디를 입력해 주십시오.");
        return false;
    }
	...
	*/
	if(f.chk_all_mb.value && (f.cp_sms_send.checked || f.cp_email_send.checked)) {
        if(!confirm("전체회원에게 쿠폰발행알림을 발송하시겠습니까?"))
            return false;
    }
    return true;
