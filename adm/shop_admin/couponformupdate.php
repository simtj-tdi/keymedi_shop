...
if($w == '') {
    ...
    if($_POST['chk_all_mb']=="1") {
        $mb_id = '전체회원';
    }else if($_POST['chk_all_mb']=="2") {
        $mb_id = '산부인과 협동조합';
    }else if($_POST['chk_all_mb']=="3") {
        $mb_id = '메디포털';
    }else if($_POST['chk_all_mb']=="4") {
        $mb_id = '피부비만';
    } else {
        $sql = " select mb_id from portal.g5_member where mb_id = '{$_POST['mb_id']}' and mb_leave_date = '' and mb_intercept_date = '' ";
        $row = sql_fetch($sql);

	    #$sqld = "select mb_id from shop_skin.g5_member where mb_id = '{$_POST['mb_id']}' and mb_leave_date = '' and mb_intercept_date = '' ";
        $sqld = "select mb_id from shop.g5_member where mb_id = '{$_POST['mb_id']}' and mb_leave_date = '' and mb_intercept_date = '' ";
		$rowd = sql_fetch($sqld); 
        if(!$row['mb_id'] && !$rowd['mb_id'])
