...
if($site_code=="shop_skin" || $site_code=="portal"){
	$sql_common = " from {$site_code}.g5_member ";
...
if($_GET['mb_id'])
    $sql_where .= " and mb_id like '%$mb_id%' ";
    ...
    <div id="scp_list_find">
		...
		<select name="site_code" id="site_code">
			<option value="">::사이트구분::</option>
				<option value="shop" <?=($site_code=="shop")?"selected":"" ?>>산부인과몰</option>
				<option value="portal" <?=($site_code=="portal")?"selected":"" ?>>키메디몰</option> 
				<option value="shop_skin" <?=($site_code=="shop_skin")?"selected":"" ?>>피부비만</option> 
		</select>
        <label for="mb_id">회원아이디</label>
        <input type="text" name="mb_id" id="mb_id" value="<?php echo $mb_id; ?>" class="frm_input required" required size="20">
