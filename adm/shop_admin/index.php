...
// 주문상태에 따른 합계 금액
function get_order_status_sum($status , $mb_id)
{
    global $g5;
	if($mb_id == "admin" || $mb_id == 'test03'){
		$where = "";
	}else{
		$where = " and com_id = '$mb_id' ";
	}
    $sql = " select count(*) as cnt,
                    sum(od_cart_price + od_send_cost + od_send_cost2 - od_cancel_price) as price
                from {$g5['g5_shop_order_table']}
                where od_status = '$status' {$where} ";
    $row = sql_fetch($sql);
...
// 일자별 주문 합계 금액
function get_order_date_sum($date ,$mb_id)
{
    global $g5;
	if($mb_id == "admin" || $mb_id == 'test03'){
		$where = "";
    ...
    $sql = " select sum(od_cart_price + od_send_cost + od_send_cost2) as orderprice,
                ...
                from {$g5['g5_shop_order_table']}
                where SUBSTRING(od_time, 1, 10) = '$date'  {$where} ";
    $row = sql_fetch($sql);
...
// 일자별 결제수단 주문 합계 금액
function get_order_settle_sum($date ,$mb_id)
{
    global $g5, $default;
	if($mb_id == "admin" || $mb_id == 'test03'){
		$where = "";
    ...
    $sql = " select sum(od_cart_coupon + od_coupon + od_send_coupon) as price,
                ...
                from {$g5['g5_shop_order_table']}
                where SUBSTRING(od_time, 1, 10) = '$date'
                  and ( od_cart_coupon > 0 or od_coupon > 0 or od_send_coupon > 0 ) {$where} ";
    $row = sql_fetch($sql);
...
<div class="sidx">
    <section id="anc_sidx_ord">
        ...
        for($i=6; $i>=0; $i--) {
            ...
            $x_val[] = $date;
            $arr_order[] = get_order_date_sum($date,$member[mb_id]);
        }
    ...
    <div id="sidx_stat">
        <section id="anc_sidx_act">
            ...
            <div id="sidx_take_act" class="tbl_head01 tbl_wrap">
                ...
                <tr>
                    <?php
                    $info = get_order_status_sum('주문',$member['mb_id']);
                    ?>
                ...
                <tr>
                    <?php
                    $info = get_order_status_sum('입금',$member['mb_id']);
                    ?>
                ...
                <tr>
                    <?php
                    $info = get_order_status_sum('준비',$member['mb_id']);
                    ?>
                ...
                <tr>
                    <?php
                    $info = get_order_status_sum('배송',$member['mb_id']);
                    ?>
...
<section id="anc_sidx_settle">
    ...
    <div id="sidx_settle" class="tbl_head02 tbl_wrap">
        ...
        <tr>
            ...
            for($i=($term - 1); $i>=0; $i--) {
                $date = date("Y-m-d", strtotime('-'.$i.' days', G5_SERVER_TIME));
                $info[$date] = get_order_settle_sum($date,$member['mb_id']);
