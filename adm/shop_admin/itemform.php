...
if ($w == "")
{
    ...
    if (!$it['ca_id'])
    {
        $sql = " select ca_id from {$site_fix}.{$g5['g5_shop_category_table']} order by ca_order, ca_id limit 1 ";
        $row = sql_fetch($sql);
        if (!$row['ca_id'])
...
else if ($w == "u")
{
    ...
    if ($is_admin != 'super')
    {
        $sql = " select it_id from {$site_fix}.{$g5['g5_shop_item_table']} a, {$site_fix}.{$g5['g5_shop_category_table']} b
                  where a.it_id = '$it_id'
                    and a.ca_id = b.ca_id
    ...
    $sql = " select * from {$site_fix}.{$g5['g5_shop_item_table']} where it_id = '$it_id' ";
    $it = sql_fetch($sql);
    ...
    $sql = " select * from {$site_fix}.{$g5['g5_shop_category_table']} where ca_id = '$ca_id' ";
    $ca = sql_fetch($sql);
}
else
{
...
// 재입고알림 설정 필드 추가
if(!sql_query(" select it_stock_sms from {$site_fix}.{$g5['g5_shop_item_table']} limit 1 ", false)) {
    sql_query(" ALTER TABLE `{$site_fix}.{$g5['g5_shop_item_table']}`
                    ADD `it_stock_sms` tinyint(4) NOT NULL DEFAULT '0' AFTER `it_stock_qty` ", true);
}

// 추가옵션 포인트 설정 필드 추가
if(!sql_query(" select it_supply_point from {$site_fix}.{$g5['g5_shop_item_table']} limit 1 ", false)) {
    sql_query(" ALTER TABLE `{$site_fix}.{$g5['g5_shop_item_table']}`
                    ADD `it_supply_point` int(11) NOT NULL DEFAULT '0' AFTER `it_point_type` ", true);
}

// 상품메모 필드 추가
if(!sql_query(" select it_shop_memo from {$site_fix}.{$g5['g5_shop_item_table']} limit 1 ", false)) {
    sql_query(" ALTER TABLE `{$site_fix}.{$g5['g5_shop_item_table']}`
...
// 지식쇼핑 PID 필드추가
// 상품메모 필드 추가
if(!sql_query(" select ec_mall_pid from {$site_fix}.{$g5['g5_shop_item_table']} limit 1 ", false)) {
    sql_query(" ALTER TABLE `{$site_fix}.{$g5['g5_shop_item_table']}`
...
// 쿠폰적용안함 설정 필드 추가
if(!sql_query(" select it_nocoupon from {$site_fix}.{$g5['g5_shop_item_table']} limit 1", false)) {
    sql_query(" ALTER TABLE `{$site_fix}.{$g5['g5_shop_item_table']}`
                    ADD `it_nocoupon` tinyint(4) NOT NULL DEFAULT '0' AFTER `it_use` ", true);
}

// 스킨필드 추가
if(!sql_query(" select it_skin from {$site_fix}.{$g5['g5_shop_item_table']} limit 1", false)) {
    sql_query(" ALTER TABLE `{$site_fix}.{$g5['g5_shop_item_table']}`
