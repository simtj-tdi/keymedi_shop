...
// 파일삭제 & 이미지 업로드
for($i=1;$i<11;$i++){
    $it_img_del = ${"it_img".$i."_del"};

    if ($it_img_del) {
        $file_img = $it_img_dir . '/' . ${"it_img" . $i};
    ...
    if ($_FILES['it_img'.$i]['name']) {
        if($w == 'u' &&  ${"it_img".$i}) {
            $file_img = $it_img_dir . '/' . ${"it_img" . $i};
            @unlink($file_img);
            delete_item_thumbnail(dirname($file_img), basename($file_img));
        }
        ${"it_img".$i} = it_img_upload($_FILES['it_img'.$i]['tmp_name'], $_FILES['it_img'.$i]['name'], $it_img_dir.'/'.$it_id);
    }
}

if ($w == "" || $w == "u")
{
    @set_cookie("ck_ca_id1", $ca_id_1, time() + 86400*31);
    @set_cookie("ck_ca_id12", $ca_id_12, time() + 86400*31);
    @set_cookie("ck_ca_id13", $ca_id_13, time() + 86400*31);
    @set_cookie("ck_ca_id2", $ca_id_2, time() + 86400*31);
    @set_cookie("ck_ca_id22", $ca_id_22, time() + 86400*31);
    @set_cookie("ck_ca_id23", $ca_id_23, time() + 86400*31);
    @set_cookie("ck_ca_id3", $ca_id_3, time() + 86400*31);
    @set_cookie("ck_ca_id32", $ca_id_32, time() + 86400*31);
    @set_cookie("ck_ca_id33", $ca_id_33, time() + 86400*31);
    @set_cookie("ck_maker", stripslashes($it_maker), time() + 86400*31);
    @set_cookie("ck_origin", stripslashes($it_origin), time() + 86400*31);
}

if(isset($ca_id_13)) $ca_id = $ca_id_13;
else $ca_id = $ca_id_12;
if(isset($ca_id_23)) $ca_id2 = $ca_id_23;
else $ca_id2 = $ca_id_22;
if(isset($ca_id_33)) $ca_id3 = $ca_id_33;
else $ca_id3 = $ca_id_32;
...
if($option_count) {
    ...
    for($i=0; $i<$option_count; $i++) {
        ...
        if($opt_val[0]) $opt1_cnt++;
        if($opt_val[1]) $opt2_cnt++;
        if($opt_val[2]) $opt3_cnt++;
    }

    if($opt1_subject && $opt1_cnt) {
        $it_option_subject = $opt1_subject;
        if($opt2_subject && $opt2_cnt) $it_option_subject .= ','.$opt2_subject;
        if($opt3_subject && $opt3_cnt) $it_option_subject .= ','.$opt3_subject;
    }
}
...
if ($w == "")
{
    ...
    if($t_it_id) alert('상품 코드는 영문자, 숫자, -, _ 만 사용할 수 있습니다.');

    $sql_common .= " , it_time = '".G5_TIME_YMDHIS."' ";
    $sql_common .= " , it_update_time = '".G5_TIME_YMDHIS."' ";
    $sql = " insert {$g5['g5_shop_item_table']} set it_id = '$it_id', $sql_common	";
}
else if ($w == "u")
{
    $sql_common .= " , it_update_time = '".G5_TIME_YMDHIS."' ";
    $sql = " update {$g5['g5_shop_item_table']} set $sql_common where it_id = '$it_id' ";
}
