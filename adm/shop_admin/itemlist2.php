...
if ($stx != "" || $save_stx!= "") {
    ...
    if($sfl == "it_8" || $sfl == "it_9" || $sfl == "it_id") {
        //$sql_search .= " $where (select $sfl from g5_shop_item where it_id = a.it_8)  = '$stx' ";
        $sql_search .= " $where $sfl = '$search_wd' ";
        $where = " and ";
    }else if ($sfl != "") {
        $sql_search .= " $where (select $sfl from g5_shop_item where it_id = a.it_8) like '%$search_wd%' ";
        $where = " and ";
    }
    if ($save_stx != $search_wd)
        $page = 1;
}
...
if ($stx2 != "") {
    if($sfl == "it_8" || $sfl == "it_9" || $sfl == "it_id") {
        ...
        $sql_search .= " $where (select $sfl2 from g5_shop_item where it_id = a.it_8) like '%$stx2%' ";
        $where = " and ";
    }
    if ($save_stx != $stx2)
        $page = 1;
...
if ($sca != "") {
    ...
    $sql_search .= " $where ((select ca_id from g5_shop_item where it_id = a.it_8) like '$sca%' or (select ca_id2 from g5_shop_item where it_id = a.it_8) like '$sca%' or (select ca_id3 from g5_shop_item where it_id = a.it_8) like '$sca%') ";
...
$sql  = " select * 
			, (select ca_id from g5_shop_item where it_id = a.it_8) as mca_id 
			, (select ca_id2 from g5_shop_item where it_id = a.it_8) as mca_id2
			, (select ca_id3 from g5_shop_item where it_id = a.it_8) as mca_id3
           $sql_common
           $sql_order
...
<form name="flist" class="local_sch02 local_sch">
    ...
    <select name="sfl" id="sfl">
        ...
        <option value="it_id" <?php echo get_selected($sfl, 'it_id'); ?>>셀러상품코드</option>
...
<form id="frm_research"  style="padding-left: 20px; display: <?=$re_display?>;" class="local_sch">
    ...
    <select name="sfl2" id="sfl2">
        ...
        <option value="it_8" <?php echo get_selected($sfl2, 'it_8'); ?>>마스터코드</option>
        <option value="it_id" <?php echo get_selected($sfl2, 'it_id'); ?>>셀러상품코드</option>
        <option value="it_9" <?php echo get_selected($sfl2, 'it_9'); ?>>업체코드</option>
...
<div class="btn_add01 btn_add">
    <? if($is_admin) { ?>
        <a href="./itemexcel_down2.php?m_class=<?=$m_class?>&sql=<?=$encrypted?>">엑셀다운로드</a>
    <? } ?>
    <a href="./itemexcel_up.php?sql=<?=$encrypted?>" onclick="return excelform(this.href);" target="_blank">상품일괄수정</a>

    <a href="./itemexcel2.php?sql=<?=$encrypted?>" onclick="return excelform(this.href);" target="_blank">상품일괄등록</a>
</div>
    ...
    <div class="tbl_head02 tbl_wrap">
        <table>
            ...
            <tr>
                ...
                <th scope="col" rowspan="3" width="5%"><?php echo subject_sort_link('it_id', 'sca='.$sca); ?>마스터코드</a></th>

                    <th scope="col" rowspan="3" width="5%">셀러상품코드</th>

                <th scope="col" rowspan="3" width="4%">업체코드</th>
                ...
                <th scope="col" <? if($is_admin){?>colspan="5"<?}else{?>colspan="4"<?}?>>분류</th>
                ...
                <th scope="col" rowspan="3" width="5%">관리</th>
            </tr>
            <tr>
                ...
                <? } ?>
                <? if($is_admin){?><th scope="col" id="th_skin">PC스킨</th><? }?>
            </tr>
            <tr>
                <? if($is_admin){?>
...
//        echo "select it_name,it_2,it_5, it_6, it_maker,it_cust_price from {$g5['g5_shop_item_table']} where it_id = '{$row[it_8]}'";

                $rows = sql_fetch("select it_name,it_2,it_5, it_6, it_maker,it_cust_price from {$g5['g5_shop_item_table']} where it_id = '{$row[it_8]}'");
                ...
                <tr class="<?php echo $bg; ?>">
                    ...
                    <td rowspan="3" style="width:100px;text-align:center;">
                        ...
                        <span style='color:red;'><?php echo $row['it_8']; ?></span>
                    </td>

                        <td rowspan="3" class="td_num"><?php echo $row['it_id']; ?> </td>
                  
                    <td rowspan="3" class="td_num"><?php echo $row['it_9']; ?> </td>
                    ...
                    <td <? if($is_admin){?>colspan="5"<?}else{?>colspan="4"<?}?>>
                        <label for="ca_id_<?php echo $i; ?>" class="sound_only"><?php echo get_text($row['it_name']); ?> 기본분류</label>
                        <select name="ca_id[<?php echo $i; ?>]" id="ca_id_<?php echo $i; ?>" <? if(!$is_admin) {?>disabled<? }?> style="width:240px;">
                            <?php echo conv_selected_option($ca_list, $row['mca_id']); ?>
                        </select>
                        <label for="ca_id2_<?php echo $i; ?>" class="sound_only"><?php echo get_text($row['it_name']); ?> 2차분류</label>
                        <select name="ca_id2[<?php echo $i; ?>]" id="ca_id2_<?php echo $i; ?>" <? if(!$is_admin) {?>disabled<? }?> style="width:240px;">
                            <?php echo conv_selected_option($ca_list, $row['mca_id2']); ?>
                        </select>
                        <label for="ca_id3_<?php echo $i; ?>" class="sound_only"><?php echo get_text($row['it_name']); ?> 3차분류</label>
                        <select name="ca_id3[<?php echo $i; ?>]" id="ca_id3_<?php echo $i; ?>" <? if(!$is_admin) {?>disabled<? }?> style="width:240px;">
                            <?php echo conv_selected_option($ca_list, $row['mca_id3']); ?>
                        </select>
                        <? if(!$is_admin) {?>
                            <input type="hidden" name="ca_id[<?php echo $i; ?>]" value="<?=$row['mca_id']?>" />
                            <input type="hidden" name="ca_id2[<?php echo $i; ?>]" value="<?=$row['mca_id2']?>" />
                            <input type="hidden" name="ca_id3[<?php echo $i; ?>]" value="<?=$row['mca_id3']?>" />
                        <? } ?>
                    </td>

                    <td rowspan="3" class="td_mngsmall"><?php echo $rows['it_5']; ?></td>
                    <td rowspan="3" class="td_mngsmall"><?php echo $rows['it_6']; ?></td>
                    <td rowspan="3" class="td_mngsmall"><?php echo $rows['it_maker']; ?></td>
                    <?
                    $min = sql_fetch("select min(it_price) as price from {$g5['g5_shop_item_table']} where it_8 = '{$row['it_8']}' and it_use = '1' and it_soldout = '0' ");
                    ?>
                    <td rowspan="3" class="td_mngsmall"><?=number_format($min[price]) ?></td>
                    <td rowspan="3" class="td_mngsmall"><?php echo $rows['it_2']; ?></td>
                    ...
                    <?}else{?>
                        <input type="hidden" name="it_order[<?php echo $i; ?>]" value="<?php echo $row['it_order']; ?>" id="order_<?php echo $i; ?>" >
                    <? }?>
                    <td rowspan="3" class="td_chk">
                    ...
                    <td rowspan="3" class="td_mng">
                        ...
                        <? }else if($row['it_10'] == $member[mb_id]){ ?>
                            <a href="./itemform2.php?w=u&amp;it_id=<?php echo $row['it_id']; ?>&amp;ca_id=<?php echo $row['ca_id']; ?>&amp;<?php echo $qstr; ?>&amp;m_class=<?=$m_class?>"  class="btn btn-primary btn-xs"><span class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'],250, "")); ?> </span>수정</a>
                        <? } ?>
                ...
                <tr class="<?php echo $bg; ?>">
                    <td rowspan="2" class="td_img"><a href="<?php echo $href; ?>"><?php echo get_it_image($row['it_8'], 50, 50); ?></a></td>
                    <td headers="th_pc_title" rowspan="2" class="td_input">
                        <label for="name_<?php echo $i; ?>" class="sound_only">상품명</label>
                        <input type="text" name="it_name[<?php echo $i; ?>]" value="<?php echo htmlspecialchars2(cut_str($rows['it_name'],250, "")); ?>" id="name_<?php echo $i; ?>" class="frm_input " size="30" <?if(!$is_admin){?>readonly<?}?>>
                    </td>
                    ...
                    <td headers="th_camt" class="td_numbig td_input">
                        <? if($is_admin) {?>
                            ...
                            <input type="text" name="it_cust_price[<?php echo $i; ?>]" value="<?php echo $rows['it_cust_price']; ?>" id="cust_price_<?php echo $i; ?>" class="frm_input sit_camt" size="7">
                        <? }else{?>
