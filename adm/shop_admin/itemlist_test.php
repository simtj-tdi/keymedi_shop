...
<form name="flist" class="local_sch02 local_sch">
    ...
    <p>
        ...
        <select name="sca1" id="sca1" onchange="ajax_cate(this.value,'sca2','2','sca2');">
        ...
        <select name="sca2" id="sca2" onchange="ajax_cate(this.value,'sca3','3','sca3');">

        </select>
        <select name="sca3" id="sca3" onchange="ajax_cate(this.value,,'3');">
    ...
    <p>
        <!-- mb_datetime  -->등록기간 : <input type="text" name="mb_s_date" id="mb_s_date" value="<?= $mb_s_date ?>"
                                           class="frm_input"/> ~ <input type="text" name="mb_e_date" id="mb_e_date"
                                                                        value="<?= $mb_e_date ?>" class="frm_input"/>
...
<select name="s_mas" id="s_mas">
    ...
    <input type="checkbox" name="re_search" id="re_search" onclick="chk_research(this);" <? if ($stx2) {
        echo "checked";
    } ?>>&nbsp;결과내 검색
...
<div class="btn_add01 btn_add">
    ...
    <? if ($is_admin) { ?>
        <a href="./itemexcel_down.php?sql=<?= $encrypted ?>">엑셀다운로드</a>
    <? } ?>
    <a href="./itemform.php">마스터상품등록</a>
    <a href="./itemexcel.php" onclick="return excelform(this.href);" target="_blank">상품일괄등록</a>
</div>

<form name="fitemlistupdate" method="post" action="./itemlistupdate_test.php" onsubmit="return fitemlist_submit(this);"
      autocomplete="off">
    ...
    <div class="tbl_head02 tbl_wrap">
        <table>
            ...
            <tr>
                ...
                <th scope="col" rowspan="3" width="5%"><?php echo subject_sort_link('it_id', 'sca=' . $sca); ?>마스터
                    코드</a></th>
                <? if ($is_admin) { ?>
                ...
                <? } ?>
                <th scope="col" rowspan="3" width="2%"><?php echo subject_sort_link('it_use', 'sca=' . $sca, 1); ?>
                    승인</a><br><input type="checkbox" name="chkall2" value="1" id="chkall2"
                                     onclick="check_all2(this.form)"></th>
                ...
                <th scope="col" rowspan="3" width="8%"><?php echo subject_sort_link('it_maker', 'sca=' . $sca); ?>
                    제조사</a></th>
                ...
                <? if ($is_admin) { ?>
                    <th scope="col" rowspan="3" width="2%"><?php echo subject_sort_link('it_order', 'sca=' . $sca); ?>
                        순서</a></th>
                    <th scope="col" rowspan="3"
                        width="2%"><?php echo subject_sort_link('it_soldout', 'sca=' . $sca, 1); ?>품절</a></th>
                <? } ?>
                <th scope="col" rowspan="3" width="2%"><?php echo subject_sort_link('it_hit', 'sca=' . $sca, 1); ?>
                    조회</a></th>
                <th scope="col" rowspan="3" width="8%">관리</th>
            </tr>
            <tr>
                <th scope="col" rowspan="2" id="th_img">이미지</th>
                <th scope="col" rowspan="2" id="th_pc_title"><?php echo subject_sort_link('it_name', 'sca=' . $sca); ?>
                    상품명</a></th>
                <? if ($is_admin) { ?>
                    <th scope="col" id="th_amt"><?php echo subject_sort_link('it_price', 'sca=' . $sca); ?>판매가격</a></th>
                    <th scope="col" id="th_camt"><?php echo subject_sort_link('it_cust_price', 'sca=' . $sca); ?>
                        시중가격</a></th>
            ...
            <? if ($is_admin) { ?>
                <tr>
                    ...
                    <th scope="col" id="th_qty"><?php echo subject_sort_link('it_stock_qty', 'sca=' . $sca); ?>
                        재고</a></th>
            ...
            for ($i = 0; $row = sql_fetch_array($result); $i++) {
                ...
                $_row_ca_id_1 = substr($row['ca_id'], 0, 2);
                $_row_ca_id_12 = substr($row['ca_id'], 0, 4);
                $_row_ca_id_13 = substr($row['ca_id'], 0, 6);

                $_row_ca_id_2 = substr($row['ca_id2'], 0, 2);
                $_row_ca_id_22 = substr($row['ca_id2'], 0, 4);
                $_row_ca_id_23 = substr($row['ca_id2'], 0, 6);

                $_row_ca_id_3 = substr($row['ca_id3'], 0, 2);
                $_row_ca_id_32 = substr($row['ca_id3'], 0, 4);
                $_row_ca_id_33 = substr($row['ca_id3'], 0, 6);
                ...
                <tr class="<?php echo $bg; ?>">
                    <? if ($is_admin) { ?>
                        <td rowspan="3" class="td_chk">
                            <label for="chk_<?php echo $i; ?>"
                                   class="sound_only"><?php echo get_text($row['it_name']); ?></label>
				 ...
				 <?php echo $row['it_id']; ?>
                    ...
                    for($c=1;$c<4;$c++){
                    ?>
                        <select name="ca_id_<?=$i.$c?>" id="ca_id_<?=$i.$c?>" onchange="ajax_cate(this.value,'ca_id_<?=$i.$c?>2','2','');">
                            <option value="">1차분류</option>
                            <?php
                            $sql1 = " select ca_id, ca_name from {$site_fix}.{$g5['g5_shop_category_table']} where LENGTH(ca_id) = '2'  order by ca_id , ca_order ";
                            $result1 = sql_query($sql1);
                            for ($r = 0; $row1 = sql_fetch_array($result1); $r++) {
                        ...
                        for ($j = 2; $j <= 3; $j++) {
                            $k=$j+1;
                            $_cateId = "ca_id_".$i.$c.$j;
                            $sendId = "ca_id_".$i.$c.$k;

                            $onChange =  $j != 3 ? "onchange=\"ajax_cate(this.value,'".$sendId."','3','".$sendId."');\"" : "" ;
                            ?>
                        ...
                        if (${"_row_ca_id_".$c}){
                            ...
                            if(strlen(${"_row_ca_id_".$c.$cs}) > 2 && strlen(${"_row_ca_id_".$c.$cs}) < 5){
                            echo "<script>
                                    document.getElementById('sca_ck').value = '" . ${"_row_ca_id_".$c.$cs} . "';
                                    ajax_cate('" . ${"_row_ca_id_".$c} . "','ca_id_".$i.$c."2','2','" . ${"_row_ca_id_".$c.$cs} . "');
                                  </script>";
                            }
                        }
                        ...
                        if (${"_row_ca_id_".$c.$cs}){
                            ...
                            if(strlen(${"_row_ca_id_".$c.$ct}) > 4  && strlen(${"_row_ca_id_".$c.$cs}) < 7){
                            echo "<script>
                                    document.getElementById('sca_ck').value = '" . ${"_row_ca_id_" . $c . $ct} . "';
                                    ajax_cate('" . ${"_row_ca_id_" . $c . $cs} . "','ca_id_".$i.$c."3','3','" . ${"_row_ca_id_" . $c . $ct} . "');
                                  </script>";
                            }
                        }
                        echo "<br>";
                    ...
                    <? if ($is_admin) { ?>
                        <td rowspan="3" class="td_mngsmall">
                            ...
                            <input type="text" name="it_order[<?php echo $i; ?>]"
                                   value="<?php echo $row['it_order']; ?>" id="order_<?php echo $i; ?>"
                        ...
                        <td rowspan="3" class="td_chk">
                            ...
                            <input type="checkbox"
                                   name="it_soldout[<?php echo $i; ?>]" <?php echo($row['it_soldout'] ? 'checked' : ''); ?>
                                   value="1" id="soldout_<?php echo $i; ?>">
                        </td>
                    <? } ?>
                    <td rowspan="3" class="td_num"><?php echo $row['it_hit']; ?></td>
                    <td rowspan="3" class="td_mng">
                        <? if ($row['it_8'] == "") { ?>
                            <a href="./itemcopy.php?it_id=<?php echo $row['it_id']; ?>&amp;ca_id=<?php echo $row['ca_id']; ?>"
                               class="itemcopy btn btn-info btn-xs" target="_blank"><span
                                        class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'], 250, "")); ?> </span>상품등록</a>
                        <? } ?>
                        <? if ($is_admin) { ?>
                            <a href="./itemform.php?w=u&amp;it_id=<?php echo $row['it_id']; ?>&amp;ca_id=<?php echo $row['ca_id']; ?>&amp;<?php echo $qstr; ?>&amp;m_class=<?= $m_class ?>"
                               class="btn btn-primary btn-xs"><span
                                        class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'], 250, "")); ?> </span>수정</a>
                        <? } ?>
                        <a href="<?php echo $href; ?>" target="_blank" class="btn btn-dark btn-xs"><span
                                    class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'], 250, "")); ?> </span>보기</a>
                    </td>
                </tr>
                <tr class="<?php echo $bg; ?>">
                    <td rowspan="2" class="td_img"><a
                                href="<?php echo $href; ?>"><?php echo get_it_image($row['it_id'], 50, 50); ?></a></td>
                    <td headers="th_pc_title" rowspan="2" class="td_input">
                        <label for="name_<?php echo $i; ?>" class="sound_only">상품명</label>
                        <input type="text" name="it_name[<?php echo $i; ?>]"
                               value="<?php echo htmlspecialchars2(cut_str($row['it_name'], 250, "")); ?>"
                               id="name_<?php echo $i; ?>" required class="frm_input required" size="30">
                    </td>
                    <? if ($is_admin) { ?>
                        <td headers="th_amt" class="td_numbig td_input">
                            <label for="price_<?php echo $i; ?>" class="sound_only">판매가격</label>
                            <input type="text" name="it_price[<?php echo $i; ?>]"
                                   value="<?php echo $row['it_price']; ?>" id="price_<?php echo $i; ?>"
                                   class="frm_input sit_amt" size="7">
                        </td>
                        <td headers="th_camt" class="td_numbig td_input">
                            <label for="cust_price_<?php echo $i; ?>" class="sound_only">시중가격</label>
                            <input type="text" name="it_cust_price[<?php echo $i; ?>]"
                                   value="<?php echo $row['it_cust_price']; ?>" id="cust_price_<?php echo $i; ?>"
                ...
                <tr class="<?php echo $bg; ?>">
                    <? if ($is_admin) { ?>
                        ...
                        <td headers="th_qty" class="td_numbig td_input">
                            ...
                            <input type="text" name="it_stock_qty[<?php echo $i; ?>]"
                                   value="<?php echo $row['it_stock_qty']; ?>" id="stock_qty_<?php echo $i; ?>"
...
<script>
    ...
    <? if($sca1){ ?>
    ajax_cate(<?=$sca1?>, <?=$sca2?>, "2");
    <? } ?>
    <? if($sca2){ ?>
    ajax_cate(<?=$sca2?>, <?=$sca3?>, "3");
    <? } ?>
</script>
<?php
