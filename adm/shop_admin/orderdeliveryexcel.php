...
if (phpversion() >= '5.2.0') {
    ...
    $headers = array('주문번호', '주문자명', '주문자전화1', '주문자전화2', '배송자명', '배송지전화1', '배송지전화2', '배송지주소', '배송회사', '운송장번호', '주소변경유무');
    ...
    for($i=1; $row=sql_fetch_array($result); $i++) {
        ...
        if(substr($row['od_id'],0,2) == "20"){
            $form_men = get_member($row['mb_id']);
        }else{
            $form_men = sql_fetch("select * from portal.g5_member where mb_id = '{$row['mb_id']}' " );
        }

        $_chk_change = $row[od_b_addr1] != $form_men[mb_6] ? "변경" : "동일" ;
    ...
    // Put Excel data
    $data = array('주문번호', '주문자명', '주문자전화1', '주문자전화2', '배송자명', '배송지전화1', '배송지전화2', '배송지주소', '배송회사', '운송장번호', '주소변경유무');
    ...
    for($i=1; $row=sql_fetch_array($result); $i++) {
        ...
        $_chk_change = $row[od_addr1] != $form_men[mb_6] ? "변경" : "동일" ;
        ...
        $worksheet->write($i, 10, $_chk_change);
    }
