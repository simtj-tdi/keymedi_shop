...
$sql_cc = sql_fetch("select sum(od_cart_coupon) as od_cart_coupon_sum from (
	select (select sum(cp_price) from {$site_fix}{$g5['g5_shop_cart_table']} c where c.od_id = m.od_id ) as od_cart_coupon from g5_shop_order 
	m where mb_id = '$od[mb_id]' and pod_id = '$od[pod_id]'
) mm ");
...
if($member['mb_level']=='7'){
    $block_chk = "disabled='disabled'";
    $block_txt = "readonly style='background-color: #BBDEFB;'";
}else{
    $block_chk = "";
    $block_txt = "";
}
...
$html_receipt_chk = '<input type="checkbox" id="od_receipt_chk" value="'.$od['od_misu'].'" onclick="chk_receipt_price()" '.$block_chk.'>
...
<section id="anc_sodr_list">
	...
	<input type="hidden" name="pod_id" value="<?php echo $od['pod_id']; ?>">
	...
	<input type="hidden" name="site_fix" value="<?php echo $site_fix;?>">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) { 
            ...
            <tr>
				...
				<td class="td_mngsmall"><?=$it_maker[it_5]?></td>
				<td class="td_mngsmall"><?=$it_maker[it_6]?></td>
				<td class="td_mngsmall"><?=$it_maker[it_maker]?></td>
...
<section id="anc_sodr_pay">
    ...
    $amount['order'] = $od['od_cart_price'] + $od['od_send_cost'] + $od['od_send_cost2'];
	 ...
	 // 쿠폰금액
    $amount['coupon'] = $od['od_cart_coupon'] + $od['od_coupon'] + $od['od_send_coupon'] + $sql_cc['od_cart_coupon_sum'];


    // 입금액 = 결제금액 - 포인트  - 쿠폰
    $amount['receipt'] = $od['od_receipt_price'] - $od['od_receipt_point'] - $amount['coupon'];
    ...
    <div class="compare_wrap">
        ...
        <section id="anc_sodr_chk" class="compare_left">
            ...
            <div class="tbl_frm01">
                ...
                <tr>
					...
					<td><?php echo display_price($amount['receipt']); ?></td>
                </tr>
                <tr>
                    <th scope="row">입금자</th>
                ...
                <?php } ?>
                <tr>
                    <th scope="row">주문금액할인<br>(주문쿠폰+상품쿠폰)</th>
                    <td><?php echo display_price($od['od_coupon'] + $sql_cc[od_cart_coupon_sum]); ?></td>
                </tr>
                <tr>
                    <th scope="row">포인트</th>
        ...
        <section id="anc_sodr_paymo" class="compare_right">
            ...
            <div class="tbl_frm01">
                ...
                <tbody>
                <?php
                $_block_chk = $member['mb_level'] == '7' ? "style='display:none';" : "" ;
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="text" name="od_receipt_price" value="<?php echo $amount['receipt']; ?>" id="od_receipt_price" class="frm_input" <?=$block_txt?>> 원
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        ...
                        <br>
                        <?php } ?>
                        <input type="text" name="od_deposit_name" value="<?php echo get_text($od['od_deposit_name']); ?>" id="od_deposit_name" class="frm_input" <?=$block_txt?>>
                    </td>
                </tr>
                <tr>
                    <th scope="row"><label for="od_receipt_time">입금 확인일시</label></th>
                    <td>
                        <input type="checkbox" name="od_bank_chk" id="od_bank_chk" value="<?php echo date("Y-m-d H:i:s", G5_SERVER_TIME); ?>"  <?=$block_chk?> onclick="if (this.checked == true) this.form.od_receipt_time.value=this.form.od_bank_chk.value; else this.form.od_receipt_time.value = this.form.od_receipt_time.defaultValue;">
                        <label for="od_bank_chk">현재 시간으로 설정</label><br>
                        <input type="text" name="od_receipt_time" value="<?php echo is_null_time($od['od_receipt_time']) ? "" : $od['od_receipt_time']; ?>" id="od_receipt_time" class="frm_input" maxlength="19" <?=$block_txt?>>
                    </td>
                </tr>
                <?php } ?>
                ...
                <tr>
                    ...
                    <td>
                        <?php echo $html_receipt_chk; ?>
                        <input type="text" name="od_receipt_price" id="od_receipt_price" value="<?php echo $od['od_receipt_price']; ?>" class="frm_input" size="10" <?=$block_txt?>> 원
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        ...
                        <input type="text" name="od_receipt_time" value="<?php echo is_null_time($od['od_receipt_time']) ? "" : $od['od_receipt_time']; ?>" id="od_receipt_time" class="frm_input" size="19" maxlength="19" readonly>
                    </td>
                </tr>
                <?php } ?>
                ...
                <?php } ?>
				<tr>
                    <th scope="row"><label for="od_coupon">주문할인 쿠폰</label></th>
                    <td><input type="text" name="od_coupon" value="<?php echo $od['od_coupon']; ?>" id="od_coupon" class="frm_input" size="10" <?=$block_txt?>> 원 </td>
                </tr>
				<tr>
                    <th scope="row"><label for="od_coupon_one">개별상품 0원처리</label></th>
                    <td><input type="checkbox" name="od_coupon_one" value="1" <?=$block_chk?>> 처리하기</td>
                </tr>
                <tr>
                    <th scope="row"><label for="od_receipt_point">포인트 결제액</label></th>
                    <td><input type="text" name="od_receipt_point" value="<?php echo $od['od_receipt_point']; ?>" id="od_receipt_point" class="frm_input" size="10" <?=$block_txt?>> 점</td>
                </tr>
                <tr>
                    <th scope="row"><label for="od_cancel_price">결제취소/환불 금액</label></th>
                    <td>
                        <input type="text" name="od_cancel_price" value="<?php echo $od['od_cancel_price']; ?>" class="frm_input" size="10" <?=$block_txt?>> 원
                    </td>
                </tr>
                <tr>
