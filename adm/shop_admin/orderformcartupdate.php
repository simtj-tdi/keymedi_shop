...
for ($i=0; $i<$cnt; $i++)
{
    ...
    $sql = " select * from {$site_fix}{$g5['g5_shop_cart_table']} where od_id = '$od_id' and ct_id  = '$ct_id' ";
    $ct = sql_fetch($sql);
    if(!$ct['ct_id'])
        continue;
    ...
    if($ct['ct_qty'] != $ct_qty) {
        ...
        if($ct['ct_stock_use']) {
            if($ct['io_id']) {
                $sql = " update {$site_fix}{$g5['g5_shop_item_option_table']}
        ...
        // 수량변경
        $sql = " update {$site_fix}{$g5['g5_shop_cart_table']}
                    set ct_qty = '$ct_qty'
                    where ct_id = '$ct_id'
                      and od_id = '$od_id' ";
        sql_query($sql);
...
if(is_array($arr_it_id) && !empty($arr_it_id)) {
    ...
    foreach($unq_it_id as $it_id) {
        $sql2 = " select sum(ct_qty) as sum_qty from {$site_fix}{$g5['g5_shop_cart_table']} where it_id = '$it_id' and ct_status = '완료' ";
        $row2 = sql_fetch($sql2);
...
if (in_array($_POST['ct_status'], $status_cancel)) {
    ...
    if($row['od_count1'] == $row['od_count2']) {
            ...
            if($od['od_tno'] && ($od['od_settle_case'] == '신용카드' || $od['od_settle_case'] == '간편결제' || $od['od_settle_case'] == 'KAKAOPAY') || ($od['od_pg'] == 'inicis' && $od['od_settle_case'] == '삼성페이')) {
                switch($od['od_pg']) {
                    ...
                    case 'inicis':
                        ...
                        $cancel_msg = iconv_euckr('쇼핑몰 운영자 부분 취소');
						
						if($od[od_cart_count] == count($it_sel)){
							$price = (int)$od[od_cart_price] + (int)$od[od_send_cost];
						}
						
						

						$pay = sql_fetch("select sum(od_receipt_price) as od_receipt_price from g5_shop_order where pod_id = '$od[pod_id]' ");
						
						$confirm_price = (int)$pay[od_receipt_price] - (int)$price;
						
						$tax = round($confirm_price / 1.1);
                ...
                if($pg_res_cd == '') {
                    ...
                    $sql = " update {$g5['g5_shop_order_table']}
                                set od_refund_price = '{$od['od_receipt_price']}' , od_cancel_price = '{$od['od_receipt_price']}'
                                where od_id = '$od_id' ";
                    sql_query($sql);
                }
            }
		...
		kakao_talk("2019010300014347" , "AT" , 0 , $od['od_hp'] , $dateis , $od['od_name']  , date("Y-m-d H:i:s") , $od['od_receipt_price'], $od['od_id'],"");
        // 관리자 주문취소 로그
...
if($cancel_change) {
	...
	$sql .= " , od_cancel_price = od_receipt_price "; // juny
