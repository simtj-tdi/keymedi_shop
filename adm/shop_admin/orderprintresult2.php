...
<? while($row = sql_fetch_array($result)){
	...
	for($i = 1 ; $rowd = sql_fetch_array($res);$i++){
        ...
        $level_date = $rmem['level_datetime']!='0000-00-00 00:00:00' ? $rmem['level_datetime'] : "-" ;
        $recommend_id = $rmem['mb_recommend'] != '' ? $rmem['mb_recommend'] : "-" ;
...
?>
	<tr>
		...
		<td rowspan="<?=$sql_cnt[cnt]?>">
            <?=number_format($row[od_send_cost])?>
        </td>
		...
		<td rowspan="<?=$sql_cnt[cnt]?>">
            <?=number_format($row[od_cart_price] + $row[od_send_cost] - $row[od_receipt_point] - $row[od_coupon] - $sql_cc[od_cart_coupon_sum]+$opt_t_price)?>
        </td>
        ...
        <td rowspan="<?=$sql_cnt[cnt]?>"><?=$level_date?></td>
        <td rowspan="<?=$sql_cnt[cnt]?>"><?=$recommend_id?></td>
	</tr>
 <? }else { 
