...
if($m_cart_txt){
	...
	$slqnew = "select od_id , it_name, it_id , (select it_8 from {$g5['g5_shop_item_table']} s where s.it_id = m.it_id ) as it_8 from {$g5['g5_shop_cart_table']} m  where {$to_add_whe} ";
...
?>
	<tr>
		...
		<td rowspan="<?=$sql_cnt[cnt]?>"><?=number_format($row[od_cart_price] + $row[od_send_cost] - $row[od_receipt_point] - $row[od_coupon] - $sql_cc[od_cart_coupon_sum]+$rowd['io_price']* $rowd[ct_qty])?></td>
