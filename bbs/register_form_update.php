...
$mb_9_sub       = isset($_POST['mb_9_sub'])         ? trim($_POST['mb_9_sub'])       : "";
...
$mb_22          = isset($_POST['mb_22'])            ? trim($_POST['mb_22'])          : "";
$mb_23          = isset($_POST['mb_23'])            ? trim($_POST['mb_23'])          : "";
$mb_24          = isset($_POST['mb_24'])            ? trim($_POST['mb_24'])          : "";
$mb_25          = isset($_POST['mb_25'])            ? trim($_POST['mb_25'])          : "";

$mb_26          = isset($_POST['mb_26'])            ? trim($_POST['mb_26'])          : "";
$mb_27          = isset($_POST['mb_27'])            ? trim($_POST['mb_27'])          : "";
$mb_28          = isset($_POST['mb_28'])            ? trim($_POST['mb_28'])          : "";
$mb_29          = isset($_POST['mb_29'])            ? trim($_POST['mb_29'])          : "";
$mb_30          = isset($_POST['mb_30'])            ? trim($_POST['mb_30'])          : "";

$mb_v			= isset($_POST['mb_v'])            ? trim($_POST['mb_v'])          : "";
$mb_shop		= isset($_POST['mb_shop'])            ? trim($_POST['mb_shop'])          : "";

$mb_where          = isset($_POST['mb_where'])            ? trim($_POST['mb_where'])          : "";
...
$pw_date2 = date("Y-m-d H:i:s",strtotime("+180 day",strtotime(date("Y-m-d"))));
...
if ($w == '') {
	...
	//kakao_talk("2019040500016021" , "AT" , "1" , $mb_hp , "00000000000000" , $mb_name , "", "", "");
	
    if ($config['cf_use_recommend'] && $mb_recommend){
        insert_point($mb_recommend, $config['cf_recommend_point'], $mb_id.'의 추천인', '@member', $mb_recommend, $mb_id.' 추천');
		insert_point($mb_id, $config['cf_recommend_point2'], $mb_recommend.'의 추천함', '@member', $mb_recommend, $mb_id.' 추천함');
...
if (isset($_POST['del_mb_icon2'])) {
    @unlink($mb_dir.'/'.$mb_id."_saup");
	@unlink($mb_dir.'/'.$mb_id."_saup.pdf");
}
...
if (isset($_FILES['mb_icon']) && is_uploaded_file($_FILES['mb_icon']['tmp_name'])) {
    if (preg_match("/(\.gif|jpg|png|bmp|JPG|PNG)$/i", $_FILES['mb_icon']['name'])) {
        ...
        if ($_FILES['mb_icon']['size'] <= $config['cf_member_icon_size']) {
            ...
            $msg .= '의사인증파일을 '.number_format($config['cf_member_icon_size']).'바이트 이하로 업로드 해주십시오.';
        }
...
if (isset($_FILES['mb_icon2']) && is_uploaded_file($_FILES['mb_icon2']['tmp_name'])) {
	...
	//	200924 - jinam23 - 사업자등록증 디비 업뎃 추가.
	$sql = " update {$g5['member_table']}
                set file_date = now()
              where mb_id = '$mb_id' ";
    sql_query($sql); 
...
if ($w == '') {
    ...
    if ($old_email != $mb_email && $config['cf_use_email_certify']) {
        ...
        <title>회원정보수정</title>
        <body>
        <form name="fregisterupdate" method="post" action="/">
		...
		if(IS_MOBILE){
			...
			<title>회원정보수정</title>
			<body>
			<form name="fregisterupdate" method="post" action="'.G5_HTTP_BBS_URL.'/content.php?co_id=0900">
			<input type="hidden" name="w" value="u">
			<input type="hidden" name="mb_id" value="'.$mb_id.'">
			<input type="hidden" name="mb_password" value="'.$tmp_password.'">
			<input type="hidden" name="is_update" value="1">
