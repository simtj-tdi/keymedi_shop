...
function get_sql_search($search_ca_name, $search_field, $search_text, $search_operator='and')
{
    ...
    $str = "";
    if ($search_ca_name)
        $str = " (ca_name = '$search_ca_name' or ca_name = '전체' )";
...
function get_category_option($bo_table='', $ca_name='')
{
    ...
    $categories = explode("|", $board['bo_category_list']/*.($is_admin?"|전체":"")*/); // 구분자가 | 로 되어 있음
    $str = "";
...
function get_point_sum($mb_id)
{
    ...
    if($config['cf_point_term'] > 0) {
        ...
        if($expire_point > 0) {
			...
			$points1 = sql_fetch("select po_mb_point from g5_point where mb_id = '{$mb_id}' order by po_id desc limit 1");
			$mb['mb_point'] = $points1['po_mb_point'];
...
function kakao_talk($temid , $talk , $concat , $tel , $date , $msg1 , $msg2, $msg3, $msg4, $msg5){
	$rtel = $tel;
	$rtel = str_replace("-", "", $rtel);
	$rtel = str_replace(".", "", $rtel);
	...
	$SVC_ID = "181227065313";
	$YELLOW_ID = "@keydocmall";
	
	if($talk == "AT"){
	$msg = "";

	if($temid=="2019010300014344"){ //카드결제
...
상담원 연결 : 070-4006-3891";

	}else if($temid=="2019010300014341"){ //가상계좌결제
		$msg = "[#병의원 전문쇼핑몰 (주)키닥]
안녕하세요. ".$msg1."님
...
- 주문번호 : $msg4
- 결제계좌 : $msg5
위 계좌로 입금 확인 후 상품이 출고됩니다.

상세내용 확인 : [마이페이지] 접속

상담원 연결 : 070-4006-3891";
	}else if($temid=="2019010300014347"){ //결제취소
		$msg = "[#병의원 전문쇼핑몰 (주)키닥]
안녕하세요. ".$msg1."님
...
상담원 연결 : 070-4006-3891";
	...
	if($temid=="2018031500009181"){
		$BUTTON_NAME = "답변 확인하기";
	}else{
		$BUTTON_NAME = "키메디 바로가기";
	}
