    ...
    function run() {
        ...
        if ($this->query) {
			...
			/*
				 ...
				 $sql_order = " ORDER BY sub_it_sum_qty DESC, it_order, it_id DESC  ";
			}
			*/
           ...
           if ($this->event) {
				...
				$sql_where2 .= " where 1 = 1 and a.ev_id = '{$this->event}' ";

				if($this->ca_id){
					 $sql_where2 .= " and ca_id like '{$this->ca_id}%' ";
				}
				if($this->com_id){
					 $sql_where2 .= " and it_10 = '{$this->com_id}' ";
				}
				$sql_where2 .= " {$sql_order} "; 

            } else {
				$sql_select = "select * from ( ";
                $sql_select .= " select {$this->fields} , (SELECT COUNT(it_id) FROM g5_shop_item WHERE it_8 = m.it_id and it_use = '1' )  AS cnt , ( SELECT SUM(it_sum_qty) FROM g5_shop_item WHERE it_8 = m.it_id AND it_use = '1') AS sub_it_sum_qty ";
                $sql_common = " from `{$g5['g5_shop_item_table']}` m  ";
				$sql_where2 = " ) a WHERE a.cnt > 0 ";
            }
            ...
            $sql = $sql_select . $sql_common . $sql_where . $sql_order . $sql_where2 . $sql_limit;


            $result = sql_query($sql);
			
			if(basename($_SERVER["PHP_SELF"])=='pharmacist.php'){
//				echo $sql;
			}

            if ($this->is_page) {
				$sql_where2 = " ) a ";
                $sql2 = "select * from (  select count(*) as cnts , (SELECT COUNT(it_id) FROM g5_shop_item WHERE it_8 = m.it_id and it_use = '1' )  AS cnt " . $sql_common . $sql_where . $sql_where2;
                $row2 = sql_fetch($sql2);
                $this->total_count = $row2['cnts'];
...
// 상품 이미지를 여부 체크
function get_it_imageChk($it_id, $width, $height=0, $anchor=false, $img_id='', $img_alt='', $is_crop=false)
{
    global $g5;
    ...
    for($i=1;$i<=10; $i++) {
        //  $file = G5_DATA_PATH.'/item/'.$row['it_img'.$i];
        $file = "/data/was/shop/data/item/".$row['it_img'.$i];
        if(is_file($file) && $row['it_img'.$i]) {
            $size = @getimagesize($file);
            if($size[2] < 1 || $size[2] > 3)
                continue;
...
// 상품 이미지를 얻는다
function get_it_image2($fixed , $datapath , $siteurl , $it_id, $width, $height=0, $anchor=false, $img_id='', $img_alt='', $is_crop=false)
{
    global $g5;
    ...
    $sql = " select it_id, it_img1, it_img2, it_img3, it_img4, it_img5, it_img6, it_img7, it_img8, it_img9, it_img10 from {$fixed}.{$g5['g5_shop_item_table']} where it_id = '$it_id' ";
    $row = sql_fetch($sql);
    ...
    if($thumb) {
        $file_url = str_replace($datapath, $siteurl,$filepath.'/'.$thumb);
		$file_url = str_replace("/item/","/data/item/",$file_url);
		//$file_url = $datapath.'/'.$thumb;
        $img = '<img src="'.$file_url.'" width="'.$width.'" height="'.$height.'" alt="'.$img_alt.'"';
    } else {
        $img = '<img src="'.G5_SHOP_URL.'/img/no_image.gif" width="'.$width.'"';
        if($height)
...
function set_cart_id($direct)
{
    ...
    if ($direct) {
        ...
        if($default['de_guest_cart_use']) {
            ...
            if(!$tmp_cart_id) {
                $tmp_cart_id = $tmp_num.substr(get_uniqid(),2);
                set_session('ss_cart_id', $tmp_cart_id);
            }
        }
		...
		$add_num = sql_fetch("select max(od_id+1) as cnt from g5_shop_order where left(od_id,16) = '$tmp_cart_id'");
		
		
		$sql = " select * from {$g5['g5_shop_cart_table']} where od_id = '$tmp_cart_id' and ct_direct = '0' and ct_status = '쇼핑' group by com_id ";
		$res = sql_query($sql);
		for($i = 0 ; $row = sql_fetch_array($res);$i++){
			
			if($add_num[cnt]){
				$add_num_tmp = $add_num[cnt]+$i;		
			}else{
				$add_num_tmp = $row[od_id].$i;
			}
			//$com_od_id = $row[od_id].$add_num_tmp;
			$com_od_id = $add_num_tmp;
			$sqlf = "update {$g5['g5_shop_cart_table']} set com_od_id = '$com_od_id' where od_id = '$tmp_cart_id' and com_id = '{$row[com_id]}' and ct_direct = '0' and ct_status = '쇼핑' ";
			sql_query($sqlf);
		}
...
function get_sendcost($cart_id, $selected=1)
{
	...
	$jeju = cut_str($member['mb_6'] ,2,"");

	$sql = "select sum(ct_price * ct_qty) as ct_price , com_id , it_id from {$g5['g5_shop_cart_table']} where com_od_id = '$cart_id'  and ct_status = '쇼핑' and ct_select = '$selected' ";
	$sum = sql_fetch($sql);
	
	$sql_m = "select mb_11 , mb_12 , mb_14 , mb_15 from shop.g5_member where mb_id = '$sum[com_id]' ";
	$res_m = sql_fetch($sql_m);

    $it_sql = "select it_sc_type from {$g5['g5_shop_item_table']} where it_id = '$sum[it_id]'";

//    echo $it_sql;
    $it_res = sql_fetch($it_sql);

    if($it_res['it_sc_type'] != 1) {

        if ($sum[ct_price] >= $res_m[mb_11]) {
            $total_send_cost = 0;
        } else {
            if ($jeju == "제주" && $res_m[mb_14] == "1") {
                $total_send_cost = $res_m[mb_15];
            } else {
