...
{
	if ($act == "seldelete2") // 선택삭제2
	{
		$ct_id2 = $_POST['ct_id'];
		if($ct_id2) { 
			$sql = " delete from {$g5['g5_shop_cart_table']} where ct_id = '$ct_id2' ";
			sql_query($sql);
		}
	}
    ...
    for($i=0; $i<$count; $i++) {
		...
		//기획상품하나만 구매가능하도록 하기
		//if($is_admin){
			if($it_id == "1584495713" || $it_id == "1584601834" || $it_id == "1621312331" ){
				$ev_row = sql_fetch("select count(*) as cnt from g5_shop_cart where it_id = '$it_id' and mb_id = '{$member['mb_id']}' ");
				if($ev_row[cnt] > 0){
					alert("이 상품은 한번만 구매하실 수 있습니다.","/");
				}
			}
		//}
        ...
        $sql = " INSERT INTO {$g5['g5_shop_cart_table']}
                        ( od_id, mb_id, it_id, com_id , com_od_id, it_name, it_sc_type, it_sc_method, it_sc_price, it_sc_minimum, it_sc_qty, ct_status, ct_price, ct_point, ct_point_use, ct_stock_use, ct_option, ct_qty, ct_notax, io_id, io_type, io_price, ct_time, ct_ip, ct_send_cost, ct_direct, ct_select, ct_select_time )
                    VALUES ";
        ...
        for($k=0; $k<$opt_count; $k++) {
            ...
            if($row2['ct_id']) {
                ...
                if ($tmp_ct_qty + $ct_qty > $tmp_it_stock_qty)
                {
					...
					alert("이미 장바구니에 담겨있는 상품입니다.","/shop/cart.php");
                }
			...
			$com_od_cnt = sql_fetch("select count(*) as cnt , com_od_id from {$g5['g5_shop_cart_table']} where od_id = '$tmp_cart_id' and com_id = '{$it['it_10']}'");
			
			//if($is_admin) echo $com_od_cnt[com_od_id];exit;
			if($com_od_cnt[cnt] > 0){
				$com_od_cnts = $com_od_cnt[com_od_id];
			}else{
				
				$com_od_cnt = sql_fetch("select max(com_od_id+1) as cnt from {$g5['g5_shop_cart_table']} where od_id = '$tmp_cart_id'");

				if($com_od_cnt[cnt] > 0){
					$com_od_cnts = $com_od_cnt[cnt];
				}else{
					$com_od_cnt = sql_fetch("select count(*) as cnt , com_od_id from {$g5['g5_shop_cart_table']} where od_id = '$tmp_cart_id'");
					$com_od_cnts = $tmp_cart_id.$com_od_cnt[cnt];
            ...
            $sql .= $comma."( '$tmp_cart_id', '{$member['mb_id']}', '{$it['it_id']}', '{$it['it_10']}' , '$com_od_cnts', '".addslashes($it['it_name'])."', '{$it['it_sc_type']}', '{$it['it_sc_method']}', '{$it['it_sc_price']}', '{$it['it_sc_minimum']}', '{$it['it_sc_qty']}', '쇼핑', '{$it['it_price']}', '$point', '0', '0', '$io_value', '$ct_qty', '{$it['it_notax']}', '$io_id', '$io_type', '$io_price', '".G5_TIME_YMDHIS."', '$REMOTE_ADDR', '$ct_send_cost', '$sw_direct', '$ct_select', '$ct_select_time' )";
            $comma = ' , ';
            $ct_count++;
        }
