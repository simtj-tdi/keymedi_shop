...
<div id="st_title">결제진행</div>
<div id="carttop_bg">
	<div class="step1 ">Step1<br>장바구니</div>
	<div class="step2 ov">Step2<br>결제진행</div>
	<div class="step3">Step3<br>주문완료</div>
</div>

<form name="forderform" id="forderform" method="post" action="<?php echo $order_action_url; ?>" autocomplete="off">

<input type="hidden" name="max_temp_point" id="max_temp_point" value="<?php echo $member['mb_point']; ?>">
<div id="sod_frm">
    <!-- 주문상품 확인 시작 { -->
	<p>※ 결제안내사항</p>
	<p style="color:red">- 의약품 포함한 공급사에는 쿠폰, <?=$point_txt?>를 적용할 수 없습니다.</p>
	<p>- 쿠폰,<?=$point_txt?> 사용 후 단순변심에 의한 취소 및 반품건은 쿠폰과 <?=$point_txt?>가 반환되지 않습니다.</p>
    <p>- <?=$point_txt?>는 1,000점 이상 보유시 사용가능합니다.</p>
    <p>주문하실 상품을 확인하세요.</p>
	<div style="margin-bottom:10px;height:auto;overflow:hidden;">
		<div style="position:relative;width:50%;float:left;">
		   &nbsp;
	   </div>
	   <div style="position:relative;width:50%;float:left;text-align:right;font-size:14px;">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <thead>
		<!-- <tr>
			<th scope="col" colspan="8"><span style="font-size:14px;">키메디 포인트 : <?php echo number_format($member['mb_point']); ?></span></th>
		</tr> -->
        <tr>
			<th scope="col" style="width:150px;">공급사</th>
            <th scope="col" style="width:150px;">상품이미지</th>
            <th scope="col">상품명</th>
            <th scope="col" style="width:100px;">총수량</th>
            <th scope="col" style="width:100px;">판매가</th>
            <th scope="col" style="width:100px;">쿠폰</th>
			<th scope="col" style="width:100px;">배송비</th>
            <th scope="col" style="width:100px;">소계</th>
            <!-- <th scope="col">포인트</th> -->
		...
		$sql_ar = "select com_id from g5_shop_cart a WHERE a.od_id = '$s_cart_id' and a.ct_select ='1' group by a.com_id  order by a.com_id ";
		$res_ar = sql_query($sql_ar);
		$t = 0;
		for($j = 0 ; $ar = sql_fetch_array($res_ar); $j++) {

		//cnt 
		$sql_cnt = " select count(a.ct_id) as cnt
                   from {$g5['g5_shop_cart_table']} a left join {$g5['g5_shop_item_table']} b on ( a.it_id = b.it_id )
                  where a.od_id = '$s_cart_id' and a.com_id = '$ar[com_id]' 
                    and a.ct_select = '1' ";
        //$sql_cnt .= " group by a.it_id ";
        $sql_cnt .= " order by a.ct_id ";
        $sql_cnt = sql_fetch($sql_cnt);
        ...
        {
			$add_row = sql_fetch("select it_name , it_5 , it_6 from {$g5['g5_shop_item_table']} where it_id = '{$row['it_8']}' ");
            ...
            if (!$goods)
            {
                ...
                $goods = preg_replace("/\'|\"|\||\,|\&|\;/", "", $add_row['it_name']);
			...
			$image = get_it_image($row['it_8'], 50, 50);
			
			

            $it_name = '<b>';
			
			$it_name .= stripslashes($add_row['it_name']);		
			if($row['it_tax']) $it_name .= " <img src='/shop/img/icon_taxs.jpg'> ";
			$it_name .= '</b>';
            ...
            if($it_options) {
				if($add_row['it_5'] && $add_row['it_6']){
                $it_name .= '<div class="sod_opt" style="color:#999999;">'.$add_row['it_5'].'/'.$add_row['it_6'].'</div>';
				}
            }
        ...
        <tr  style="position:relative;">
			<? if($i==0) {?>
			<td class="sod_img" rowspan="<?=$sql_cnt[cnt]?>"><?php echo $row['mb_nick']; ?></td>
			<? } ?>
            <td class="sod_img"><?php echo $image; ?></td>
            <td>
                <input type="hidden" name="it_id[<?php echo $t; ?>]"    value="<?php echo $row['it_id']; ?>">
                <input type="hidden" name="it_name[<?php echo $t; ?>]"  value="<?php echo get_text($row['it_name']); ?>">
                <input type="hidden" name="it_price[<?php echo $t; ?>]" value="<?php echo $sell_price; ?>">
                <input type="hidden" name="cp_id[<?php echo $t; ?>]" value="">
                <input type="hidden" name="cp_price[<?php echo $t; ?>]" value="0">
				<input type="hidden" name="com_od_id[<?php echo $t; ?>]" value="<?php echo $row['com_od_id']; ?>">
				<input type="hidden" name="com_id[<?php echo $t; ?>]" value="<?php echo $row['com_id']; ?>">
                <?php if($default['de_tax_flag_use']) { ?>
                <input type="hidden" name="it_notax[<?php echo $t; ?>]" value="<?php echo $row['it_notax']; ?>">
                <?php } ?>
                <?php
                echo $it_name;

                $opt_sql = "SELECT ct_option,io_id,io_price FROM g5_shop_cart WHERE com_od_id='{$row['com_od_id']}'";
        ...
        <?php
			...
			$t++;
			$com_od_id = $row['com_od_id'];
			$coupon_com_id =  $row['com_id'];
			...
			$sql = " select cp_id
						...
						where mb_id IN ( '{$member['mb_id']}', '{$member['mb_where']}','전체회원' )
						  ...
						  and ( cp_method = '2' and (cp_target = '$coupon_com_id' or cp_target = 'all' ) ) ";
			$res = sql_query($sql);
			
			//if($member[mb_id] == "jun2ys") echo $sql;
			...
			if($f_cp_count) {
				$f_cp_button = '<button type="button" class="cp_btn2 btn_frmline">적용</button>';		 
			}
 ...
 if ($is_member && $config['cf_use_point'])
			...
			//echo $default['de_settle_min_point'];  
?>
		<? if($temp_point > 0 && $is_tax ==0){ ?>
		<tr>
			<td colspan="8" align="right" style="background:#eee;position:relative;">
			<?=$point_txt?>사용 : <input type="number" step="100" min="0" max="<?=$member['mb_point']?>" maxlength="7" size="7" class="frm_input"  id="f_point[]" value="0" name="f_point[]" onchange="add_point(this,<?=$sell_total_price?>,this.value,'<?=$coupon_com_id?>',<?=$sell_total_price + get_sendcost($com_od_id)?>);return false;">점
			</td>
		</tr>
		<? }else{ ?>
				<input type="hidden" name="f_point[]" value="0">
		<? } ?>
		<? if($f_cp_count && $is_tax ==0){ ?>
		<tr>
			<td colspan="8" align="right" style="background:#eee;position:relative;">
				<input type="hidden" name="coupon_com_id" value="<?=$coupon_com_id?>">
				<input type="hidden" name="coupon_od_id" value="<?=$com_od_id?>">
				<input type="hidden" name="tmp_cp_id[]" value="">
				<input type="hidden" name="f_cp_id[<?=$j?>]" value="">

				<input type="hidden" name="f_p_f_price[<?=$j?>]" value="<?php echo $sell_total_price; ?>">

				<input type="hidden" name="tmp_f_point_[<?=$j?>]" id="tmp_f_point_<?=$coupon_com_id?>" value="0">

				<input type="hidden" name="f_cp_price[<?=$j?>]" id="f_cp_price_tmp_<?=$coupon_com_id?>" value="0">
				<input type="hidden" name="f_cp_tmp[<?=$j?>]" id="f_cp_tmp_<?=$coupon_com_id?>" value="0">
				<input type="hidden" name="f_f_price[<?=$j?>]" id="f_f_price_<?=$coupon_com_id?>" value="<?=$sell_total_price + get_sendcost($com_od_id)?>">
				<input type="hidden" name="f_f_p_tmp[<?=$j?>]" id="f_f_price_tmp_<?=$coupon_com_id?>" value="<?=$sell_total_price + get_sendcost($com_od_id)?>">

				주문할인쿠폰 <?php echo $f_cp_button; ?>
			</td>
		</tr>
		<!-- <tr>
			<td colspan="8" align="right" style="background:#eee;">
				주문할인금액 : <span id="od_cp_price_<?=$coupon_com_id?>">0</span>원
			</td>
		</tr> -->
		<? }else{ ?>
			    <input type="hidden" name="coupon_com_id" value="<?=$coupon_com_id?>">
		...
		<tr>
			<td colspan="8" align="right" style="background:#eee;">
				상품금액 : <?=$sell_total_price?> 
				+ 배송비 : <?=get_sendcost($com_od_id)?> 
				<span id="f_coupon_price<?=$coupon_com_id?>"></span>
				<span id="f_point_price<?=$coupon_com_id?>"><!-- - 포인트 : 0 --></span>
				<span id="f_price<?=$coupon_com_id?>"> = 결제예상금 : <?=$sell_total_price + get_sendcost($com_od_id)?></span></td>
		</tr>
		<?
		$sell_total_price = 0;

       //$send_cost = $send_cost + get_sendcost($s_cart_id);

       if($row['it_sc_type'] != 1) {
           $send_cost = $send_cost + get_sendcost($com_od_id);
       }
    ...
    $_delivery_count = sql_fetch("SELECT count(ad_id) as d_cnt FROM {$g5['g5_shop_order_address_table']} where mb_id='{$member['mb_id']}' and ad_default=1 order by ad_id desc limit 1");
    $_delivery_sql = $_delivery_count['d_cnt'] > 0 ? "and ad_default=1" : "" ;
    $_delivery_info = sql_fetch("SELECT ad_subject,ad_name,ad_tel,ad_hp,ad_zip1,ad_zip2,ad_addr1,ad_addr2,ad_addr3,ad_jibeon FROM {$g5['g5_shop_order_address_table']} where mb_id='{$member['mb_id']}' {$_delivery_sql} order by ad_id desc limit 1");

    $_order_name    = $member['mb_name'];
    $_order_phone   = $member['mb_8'];
    $_order_hp      = $member['mb_hp'];
    $_order_zip     = $member['mb_5'];
    $_order_addr1   = $member['mb_6'];
    $_order_addr2   = $member['mb_7'];
    $_order_jibeon  = $member['mb_addr_jibeon'];
    ?>

    <?php if ($goods_count) $goods .= ' 외 '.$goods_count.'건'; ?>
    <!-- } 주문상품 확인 끝 -->
	<h4 style="font-size:18px;margin-top:20px;"><?=$goods?></h4>
	<div style="position:relative;width:100%;height:135px;background:url('/img/sum_price.jpg') no-repeat;">
		<strong style="position:absolute;width:235px;left:0;top:60px;text-align:center;font-size:18px;"><?php echo number_format($tot_sell_price); ?> 원</strong>	
		<strong style="position:absolute;width:235px;left:235px;;top:60px;text-align:center;font-size:18px;"><?php echo number_format($send_cost); ?> 원</strong>
	 
		<strong style="position:absolute;width:235px;left:470px;;top:60px;text-align:center;font-size:18px;" id="ct_tot_coupon">0 원</strong>
		<strong style="position:absolute;width:235px;left:705px;;top:60px;text-align:center;font-size:18px;" id="ct_tot_point"><?php echo number_format($tot_point); ?> 점</strong>
		<?php $tot_price = $tot_sell_price + $send_cost; // 총계 = 주문상품금액합계 + 배송비 ?>
        <strong style="position:absolute;width:235px;left:940px;;top:60px;text-align:center;font-size:18px;" id="ct_tot_price"><?php echo number_format($tot_price); ?> 원</strong>
	</div>
    <!-- 주문상품 합계 시작 { -->
    ...
    <section id="sod_frm_orderer">
        <h2 style="font-size:18px;margin-top:20px;">주문하시는 분</h2>
        ...
        <div class="tbl_frm01 tbl_wrap">
            ...
            <tr>
                ...
                <td><input type="text" name="od_name" value="<?php echo get_text($member['mb_name']); ?>" id="od_name" required class="frm_input required" maxlength="20" readonly></td>
            </tr>
            ...
            <tr>
                ...
                <td><input type="text" name="od_tel" value="<?php echo get_text($member['mb_8']); ?>" id="od_tel" required class="frm_input required" maxlength="20" ></td>
            </tr>
            <tr>
                <th scope="row"><label for="od_hp">핸드폰</label></th>
                <td><input type="text" name="od_hp" value="<?php echo get_text($member['mb_hp']); ?>" id="od_hp" class="frm_input" maxlength="20" readonly></td>
            </tr>
            <tr>
                <th scope="row">주소</th>
                <td>
                    <label for="od_zip" class="sound_only">우편번호<strong class="sound_only"> 필수</strong></label>
                    <input type="text" name="od_zip" value="<?php echo $member['mb_5']; ?>" id="od_zip" required class="frm_input required" size="5" maxlength="6" readonly>
                    ...
                    <input type="text" name="od_addr1" value="<?php echo get_text($member['mb_6']) ?>" id="od_addr1" required class="frm_input frm_address required" size="51" readonly>
                    <label for="od_addr1">기본주소<strong class="sound_only"> 필수</strong></label><br>
                    <input type="text" name="od_addr2" value="<?php echo get_text($member['mb_7']) ?>" id="od_addr2" class="frm_input frm_address" size="51" readonly>
                    <label for="od_addr2">상세주소</label>
                    <br>
                    <!-- <input type="text" name="od_addr3" value="<?php echo get_text($member['mb_addr3']) ?>" id="od_addr3" class="frm_input frm_address" size="51" readonly="readonly">
            ...
            <tr>
                ...
                <td><input type="text" name="od_email" value="<?php echo $member['mb_email']; ?>" id="od_email" required class="frm_input required" size="35" maxlength="100" readonly></td>
            </tr>
			<? if($member[mb_id] == "test02") {?>
			<tr>
                <th scope="row">병원명</th>
                <td><?=$member[mb_11]?></td>
            </tr>
			<tr>
                ...
                <td><?=$member[mb_15]?> <br><span style="color:red;">* 위 사업자등록번호로 세금계산서가 발행됩니다. 정보가 상이할 경우 고객센터로 연락 주시기 바랍니다.</span></td>
            </tr>
			<? } ?>
    ...
    <section id="sod_frm_taker">
        <h2 style="font-size:18px;margin-top:20px;">받으시는 분</h2>
        ...
        <div class="tbl_frm01 tbl_wrap">
            ...
            if($is_member) {
                ...
                $addr_list .= '&nbsp;&nbsp;<a href="' . G5_SHOP_URL . '/new_orderaddress.php" id="order_address" class="btn_frmline">배송지목록</a>';
				/*
                // 기본배송지
                $sql = " select *
                            from {$g5['g5_shop_order_address_table']}
            ...
            <tr>
                ...
                <td><input type="text" name="od_b_name" id="od_b_name" required class="frm_input required" maxlength="20" readonly></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td><input type="text" name="od_b_hp" id="od_b_hp" class="frm_input" maxlength="20" readonly></td>
            </tr>
            <tr>
                <th scope="row">주소</th>
                <td id="sod_frm_addr">
                    <label for="od_b_zip" class="sound_only">우편번호<strong class="sound_only"> 필수</strong></label>
                    <input type="text" name="od_b_zip" id="od_b_zip" required class="frm_input required" size="5" maxlength="6" readonly>
                    ...
                    <input type="text" name="od_b_addr1" id="od_b_addr1" required class="frm_input frm_address required" size="51" readonly>
                    <label for="od_b_addr1">기본주소<strong class="sound_only"> 필수</strong></label><br>
                    <input type="text" name="od_b_addr2" id="od_b_addr2" class="frm_input frm_address" size="51 readonly">
                    <label for="od_b_addr2">상세주소</label>
                    <br>
                    <!-- <input type="text" name="od_b_addr3" id="od_b_addr3" readonly="readonly" class="frm_input frm_address" size="51" readonly>
    ...
    <?if($medical_check=='Y'){?>
        <script language="JavaScript">
            $('#od_sel_addr_same').attr('checked',true);
            $('#od_sel_addr_new').attr('disabled',true);
            $('#order_address').attr("disable",true);
            $('#order_address').removeAttr("href")

        </script>
    <?}else{?>
        <script language="JavaScript">
            $('#od_sel_addr_same').attr('checked',true);
            $('#od_sel_addr_new').attr('disabled',false);
            $('#order_address').prop("disable",false);

            // 배송지목록
            $("#order_address").on("click", function () {
                var url = this.href;
                window.open(url, "win_address", "left=100,top=100,width=600,height=500,scrollbars=1");
                return false;
            });
        </script>
    <?}?>
    ...
    <section id="sod_frm_pay">
        <h2 style="font-size:18px;margin-top:20px;">결제정보</h2>
...
$(function() {
	...
	$(".cp_btn2").click(function() {

		

        $cp_btn_el2 = $(this);
        $cp_row_el2 = $(this).closest("tr");
        $("#cp_frm2").remove();
        var it_id = $cp_btn_el2.closest("tr").find("input[name^=coupon_com_id]").val();
		var od_it_id = $cp_btn_el2.closest("tr").find("input[name^=coupon_od_id]").val();

        $.post(
            "./orderitemcoupon2.php",
            { it_id: it_id, od_it_id:od_it_id, sw_direct: "<?php echo $sw_direct; ?>" },
            function(data) {
                $cp_btn_el2.after(data);
            }
        );
    });
    ...
    $(document).on("click", ".cp_apply", function() {
		...
		var codes = $cp_row_el.find("input[name^='com_id']").val();  

		//document.getElementById("f_f_price_"+codes).value = ( document.getElementById("f_f_price_"+codes).value - parseInt(price));
		//document.getElementById("f_cp_tmp_"+codes).value = ( parseInt(document.getElementById("f_cp_tmp_"+codes).value) + parseInt(price));
		
		var f_f_price_full =  document.getElementById("f_f_price_"+codes).value;
		var f_f_price_full_tmp  =  document.getElementById("f_f_price_tmp_"+codes).value;
	    var f_cp_price_tmp = document.getElementById("f_cp_price_tmp_"+codes).value;
		var tot_cp_price_tmp = 0;
		var cp_price2;
		var $cp_prc = $("input[name^=cp_price]");
		var $cp_com_id = $("input[name^=com_id]");
		$cp_prc.each(function(index) { 
			if($cp_com_id.eq(index).val() == codes){
		
				cp_price2 = parseInt($cp_prc.eq(index).val()); 	 
				
				tot_cp_price_tmp += cp_price2;  
			}
		});
		
		document.getElementById("f_cp_tmp_"+codes).value = parseInt(tot_cp_price_tmp);
		

		document.getElementById("f_f_price_"+codes).value = ( document.getElementById("f_f_price_tmp_"+codes).value - parseInt(document.getElementById("f_cp_tmp_"+codes).value) - parseInt(document.getElementById("f_cp_price_tmp_"+codes).value));


		document.getElementById("f_coupon_price"+codes).innerHTML = "- 쿠폰 : "+ parseInt(parseInt(document.getElementById("f_cp_tmp_"+codes).value)+parseInt(f_cp_price_tmp)); 

		document.getElementById("f_price"+codes).innerHTML = " = 결제예상금 : "+ parseInt(parseInt(f_f_price_full_tmp) - parseInt(parseInt(document.getElementById("f_cp_tmp_"+codes).value)+parseInt(f_cp_price_tmp)));

		 calculate_total_price();
    });


	 $(document).on("click", ".cp_apply2", function() {
        ...
        var price = $el.find("input[name='f_cp_prc[]']").val(); 
        var subj = $el.find("input[name='f_cp_subj[]']").val();
		var codes = $el.find("input[name='f_cp_code[]']").val();
        var sell_price;
		
		var tmp_point = $cp_row_el2.find("input[name^=tmp_f_point_]").val();
		var tmp_price = $cp_row_el2.find("input[name^=f_p_f_price]").val();
		var tmptmp_cu = parseInt(tmp_price - tmp_point);

		if(cpcheck(cp_id) == 1){ alert("이미 선택하신 쿠폰입니다.");return false;}
		
		if(tmptmp_cu < parseInt(price)){
			alert("결제금 이상으로 사용하실 수 없습니다.");return false;
		}
		...
		//var $s_el = $cp_row_el.find(".total_price");;
        //$s_el.text(number_format(String(sell_price)));
		$cp_row_el2.find("input[name^=tmp_cp_id]").val(cp_id);
        $cp_row_el2.find("input[name^=f_cp_id]").val(cp_id);
        $cp_row_el2.find("input[name^=f_cp_price]").val(price);

		var f_price = $cp_row_el2.find("input[name^=f_f_price]").val();
		$cp_row_el2.find("input[name^=f_f_price]").val( parseInt($cp_row_el2.find("input[name^=f_f_p_tmp]").val()) - parseInt(price)  - parseInt($cp_row_el2.find("input[name^=f_cp_tmp]").val()) );
		var f_price = $cp_row_el2.find("input[name^=f_f_price]").val();
		
		var f_price_tmp = $cp_row_el2.find("input[name^=f_cp_tmp]").val();

		document.getElementById("f_coupon_price"+codes).innerHTML = "- 쿠폰 : "+parseInt(parseInt(price)+parseInt(f_price_tmp));
		//document.getElementById("od_cp_price_"+codes).innerHTML = parseInt(parseInt(price)+parseInt(f_price_tmp));

		document.getElementById("f_price"+codes).innerHTML = " = 결제예상금 : "+ f_price;

        
        $cp_btn_el2.text("변경").focus();
        if(!$cp_row_el2.find(".cp_cancel2").size())
            $cp_btn_el2.after("<button type=\"button\" class=\"cp_cancel2 btn_frmline\">취소</button>");

		//$(".cp_btn2").css("display","none");
		calculate_total_price(); 
		$("#cp_frm2").remove();
    });
    ...
    $(document).on("click", "#cp_close", function() {
	 ...
	 $(document).on("click", "#cp_close2", function() {
        $("#cp_frm2").remove();
        $cp_btn_el2.focus();
    });
    ...
    $(document).on("click", ".cp_cancel", function() {
		var $el = $(this).closest("tr");
		var cp_price = $el.find("input[name^='cp_price']").val();  
		var codes = $el.find("input[name^='com_id']").val(); 
       ...
       // calculate_total_price();
		...
		document.getElementById("f_f_price_"+codes).value = ( parseInt(document.getElementById("f_f_price_"+codes).value) + parseInt(cp_price));
		document.getElementById("f_cp_tmp_"+codes).value = parseInt(parseInt(document.getElementById("f_cp_tmp_"+codes).value) - parseInt(cp_price));
		

		var f_f_price_full =  document.getElementById("f_f_price_"+codes).value;
	    var f_cp_price_tmp = document.getElementById("f_cp_price_tmp_"+codes).value;
		document.getElementById("f_coupon_price"+codes).innerHTML = "- 쿠폰 : "+ parseInt(parseInt(f_cp_price_tmp) + parseInt(document.getElementById("f_cp_tmp_"+codes).value));
		//document.getElementById("od_cp_price_"+codes).innerHTML = parseInt(f_cp_price_tmp);
		document.getElementById("f_price"+codes).innerHTML = " = 결제예상금 : "+ f_f_price_full;
	...
	$(document).on("click", ".cp_cancel2", function() {
		 ...
		 $cp_row_el2 = $(this).closest("tr");
		$cp_row_el2.find("input[name^=tmp_cp_id]").val(""); 
        $cp_row_el2.find("input[name^=f_cp_id]").val("");        
		$cp_row_el2.find("input[name^=f_f_price]").val( parseInt($cp_row_el2.find("input[name^=f_f_price]").val()) + parseInt($cp_row_el2.find("input[name^=f_cp_price]").val()));
		$cp_row_el2.find("input[name^=f_cp_price]").val("0"); 

		var $el = $(this).closest("tr"); 
		var codes = $el.find("input[name='coupon_com_id']").val();
		 
		var f_price = $cp_row_el2.find("input[name^=f_f_price]").val();
		var f_cp_price  = $cp_row_el2.find("input[name^=f_cp_price]").val();
		var f_cp_tmp = $cp_row_el2.find("input[name^=f_cp_tmp]").val(); 

		document.getElementById("f_coupon_price"+codes).innerHTML = "- 쿠폰 : "+ parseInt(f_cp_tmp);
		//document.getElementById("od_cp_price_"+codes).innerHTML = parseInt(f_cp_tmp);
		document.getElementById("f_price"+codes).innerHTML = " = 결제예상금 : "+ f_price;
		
		$(".cp_btn2").css("display","");
        //$("#cp_frm2").remove();
        $(this).closest("tr").find(".cp_btn2").text("적용").focus();
        $(this).remove();
    ...
    gumae2baesong();
	$("input[name=ad_sel_addr]").attr('checked', 'true');
...
function calculate_total_price()
{
	...
	//var $cp_prc = $("input[name^=cp_price]");
    var $it_prc = $("input[name^=f_f_price]");
    var $cp_prc = $("input[name^=f_cp_price]");
	var $cp_prc2 = $("input[name^=f_cp_tmp]");
	var $f_point = $("input[name^=f_point]");
		

    var tot_sell_price = sell_price = tot_cp_price = tot_f_point = 0;
    var it_price, cp_price, cp_price2, f_point , it_notax;
...
function forderform_check(f)
{
    ...
    if (typeof(f.od_temp_point) != "undefined") {
        if (f.od_temp_point.value)
        {
            ...
            if (temp_point < 0) {
                alert("<?=$point_txt?>를 0 이상 입력하세요.");
                f.od_temp_point.select();
                return false;
            }

            if (temp_point > od_price) {
                alert("상품 주문금액(배송비 제외) 보다 많이 <?=$point_txt?>결제할 수 없습니다.");
                f.od_temp_point.select();
                return false;
            }

            if (temp_point > <?php echo (int)$member['mb_point']; ?>) {
                alert("회원님의 <?=$point_txt?>보다 많이 결제할 수 없습니다.");
                f.od_temp_point.select();
                return false;
            }
            ...
            if (parseInt(parseInt(temp_point / point_unit) * point_unit) != temp_point) {
                alert("<?=$point_txt?>를 "+String(point_unit)+"점 단위로 입력하세요.");
                f.od_temp_point.select();
                return false;
            }
...
function add_point(ttmp,max_p,val,codes1,tt_price){

var tot_max_p = max_p - document.getElementById("f_cp_price_tmp_"+codes1).value;

if(val==""){ 
	ttmp.value = "0";
}
var tt_price = document.getElementById("f_f_price_"+codes1).value;

var ff_point = 0;

var chk = document.getElementsByName("f_point[]");

for (i=0; i<chk.length; i++)
{ 
	ff_point = ff_point + (chk[i].value*1);
}

<? if($member['mb_point'] > 0){ ?>
	if( val > <?=$member['mb_point']?>  ){
		alert("보유<?=$point_txt?>보다 크게 입력할수 없습니다.");	
		ttmp.value = "0";
		document.getElementById("tmp_f_point_"+codes1).value = 0;
		document.getElementById("f_point_price"+codes1).innerHTML = "- <?=$point_txt?> : 0";
		document.getElementById("f_price"+codes1).innerHTML = " = 결제예상금 : "+ tt_price;
		calculate_total_price();
		return false;
	}else if(val > tot_max_p){
 		alert("상품금액보다 크게 입력할수 없습니다.");	
		ttmp.value = "0";
		document.getElementById("tmp_f_point_"+codes1).value = 0;
		document.getElementById("f_point_price"+codes1).innerHTML = "- <?=$point_txt?> : 0";
		document.getElementById("f_price"+codes1).innerHTML = " = 결제예상금 : "+ tt_price;
		calculate_total_price();
		return false;
	}else if(ff_point > <?=$member['mb_point']?>){
		alert("보유<?=$point_txt?>보다 크게 입력할수 없습니다.");	
		ttmp.value = "0";
		document.getElementById("tmp_f_point_"+codes1).value = 0;
		document.getElementById("f_point_price"+codes1).innerHTML = "- <?=$point_txt?> : 0";
		...
		document.getElementById("tmp_f_point_"+codes1).value = val;
		document.getElementById("f_f_price_"+codes1).value = tt_price;
		document.getElementById("max_temp_point").value = ( document.getElementById("max_temp_point").value - val);
		document.getElementById("f_point_price"+codes1).innerHTML = "- <?=$point_txt?> : "+val;
		document.getElementById("f_price"+codes1).innerHTML = " = 결제예상금 : "+ (tt_price-val);
...
function cpcheck(tmp){
	var chk = document.getElementsByName("tmp_cp_id[]");
	var ck = 0;
	for (i=0; i<chk.length; i++)
	{ 
		if(tmp == chk[i].value){
...
function gumaenewbaesong() {
    ...
    document.getElementById('od_b_name').readOnly = false;
    document.getElementById('od_b_tel').readOnly = false;
    document.getElementById('od_b_hp').readOnly = false;
    document.getElementById('od_b_zip').readOnly = false;
    document.getElementById('od_b_addr1').readOnly = false;
    document.getElementById('od_b_addr2').readOnly = false;
    document.getElementById('od_b_addr_jibeon').readOnly = false;
...
function gumae2baesong() {
    ...
    /*if(f.od_addr_jibeon.value!='') {
        f.od_b_addr_jibeon.value = f.od_addr_jibeon.value;
    }*/
    document.getElementById('od_b_name').readOnly = true;
    document.getElementById('od_b_tel').readOnly = true;
    document.getElementById('od_b_hp').readOnly = true;
    document.getElementById('od_b_zip').readOnly = true;
    document.getElementById('od_b_addr1').readOnly = true;
    document.getElementById('od_b_addr2').readOnly = true;
    /*if(f.od_addr_jibeon.value!='') {
        document.getElementById('od_b_addr_jibeon').readOnly = true;
    }*/
