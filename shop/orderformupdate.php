...
// 배송비가 상이함 juny
	$sql_ar = "select com_od_id from g5_shop_cart a WHERE a.od_id = '$tmp_cart_id' and a.ct_select ='1' group by a.com_id  order by a.com_id ";
	$res_ar = sql_query($sql_ar);
	$send_cost = 0;
	for($j = 0 ; $ar = sql_fetch_array($res_ar); $j++) {
		$send_cost = $send_cost + get_sendcost($ar[com_od_id]);
	}
...
if ((int)($send_cost - $tot_sc_cp_price) !== (int)($i_send_cost - $i_send_coupon)) {
    //die("Error.."); 2017-9-15 juny 배송비문제
}
...
if($_POST[oid]){
	$pod_id = $_POST[oid];
	$com_od_id_tmp = array_unique($_POST[com_od_id]);
}else{
	$pod_id = $od_id;
	$com_od_id_tmp = array_unique($_POST[com_od_id]);
}
...
if($od_pg == 'inicis') {
	/*
    $sql = " delete from {$g5['g5_shop_order_data_table']} where od_id = '$pod_id' and dt_pg = '$od_pg' ";
    sql_query($sql);
	*/
	$del_com_od_id = substr($com_od_id_tmp[0],0,16);
    $sql = " delete from {$g5['g5_shop_order_data_table']} where cart_id = '$del_com_od_id' and dt_pg = '$od_pg' ";
    sql_query($sql);
}
