...
if (G5_IS_MOBILE) {
    include_once(G5_MSHOP_PATH.'/pharmacist.php');
    return;
}
...
$sql = " select * from {$g5['g5_shop_category_table']} where ca_id = '{$_data['mall_ca_id']}' and ca_use = '1'  ";
...
if ($is_admin)
    ////echo '<div class="sct_admin"><a href="'.G5_ADMIN_URL.'/shop_admin/categoryform.php?w=u&amp;ca_id='.$ca_id.'" class="btn_admin">분류 관리11</a></div>';
    ...
    <div>
        <?
        $logo_output = image_resize_return(G5_DATA_PATH . '/file/private/'.$_data['mall_logo'],'/data/file/private/'.$_data['mall_logo'], 150, 60);
        if ($logo_output)
            echo $logo_output;
        ?>
    </div>
    <div style="margin-top: 15px; font-size: 16px; color: #6f7071;"><?=$_data['mall_word']?></div>
    <div style="margin-top: 20px;">

            <?
            $banner_output = image_resize_return(G5_DATA_PATH . '/file/private/'.$_data['mall_banner'],'/data/file/private/'.$_data['mall_banner'], 1180, 220);
            if ($banner_output){
                if($_data['mall_link']){
                    echo "<a href='".$_data['mall_link']."' target='_blank'>".$banner_output."</a>";
                }else{
                    echo $banner_output;
                }
            }
...
<?
        ...
        if ($sort != "")
            ...
            //$order_by = 'sub_it_sum_qty DESC, it_order, it_id DESC';
            $order_by = ' it_order, it_id DESC';

        $error = '<p class="sct_noitem">등록된 상품이 없습니다.</p>';

        // 리스트 스킨
        $skin_file = is_include_path_check($skin_dir.'/'.$ca['ca_skin']) ? $skin_dir.'/'.'list.mall.skin.php' : $skin_dir.'/list.mall.skin.php';

//        echo $skin_file;

        if (file_exists($skin_file)) {

            //# 2021.06.17 5*4 로 상품 노출되도록 수정 ( jira > KNXP-214 )
            $list_mod = 4;
            $list_row = 5;
            ...
            $list = new item_list($skin_file,  $list_mod, $list_row, $ca['ca_img_width'], $ca['ca_img_height']);
        ...
        {
            echo '<div align="center">'.str_replace(G5_PATH.'/', '', $skin_file).' 파일을 찾을 수 없습니다.<br>관리자에게 알려주시면 감사하겠습니다.</div>';
        }
        ?>
