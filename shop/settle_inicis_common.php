        ...
        if($row['pp_id']) {
            ...
            if($row['od_id']) {
                ...
                $sql = " update {$g5['g5_shop_order_table']}
                          ...
                          where pod_id = '{$row['od_id']}' ";
                $result = sql_query($sql, FALSE);
            }
        } else {
            // 주문서 UPDATE
            $sql = " update {$g5['g5_shop_order_table']}
                        set od_receipt_price = od_cart_price+od_send_cost,
                            od_receipt_time = '$receipt_time'
                      where pod_id = '$no_oid'
        ...
        if($result) {
            ...
            if($row['cnt'] > 0) {
				...
				*/
				...
				$sql = "select od_id from {$g5['g5_shop_order_table']} where pod_id = '$od_id' ";
				$res = sql_query($sql);
