...
<aside id="bo_vc_w">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        ?>
		<tr>
            <th scope="row"><label for="wr_1">닉네임<strong class="sound_only"> 필수</strong></label></th>
            <td><input type="text" name="wr_1" value="<?=($c_wr_1)?$c_wr_1:$member[mb_nick]?>" id="wr_1" required class="frm_input required" size="5" maxLength="20"></td>
        </tr>
        <tr>
            <th scope="row">내용</th>
            <td>
...
<section id="bo_vc">
    ...
    <article id="c_<?php echo $comment_id ?>" <?php if ($cmt_depth) { ?>style="margin-left:<?php echo $cmt_depth ?>px;border-top-color:#e0e0e0"<?php } ?>>
        <header style="z-index:<?php echo $cmt_sv; ?>">
            ...
            <?php echo $list[$i]['wr_1'] ?><?//php echo $list[$i]['name'] ?>
			<?if($is_admin) echo "(".$list[$i]['mb_id'].")"; ?>
        ...
        <?php if($list[$i]['is_reply'] || $list[$i]['is_edit'] || $list[$i]['is_del']) {
            ...
            if($w == 'cu') {
                $sql = " select wr_id, wr_content , wr_1 from $write_table where wr_id = '$c_id' and wr_is_comment = '1' ";
                $cmt = sql_fetch($sql);
...
function comment_box(comment_id, work)
{
    ...
    if (save_before != el_id)
    {
        ...
        if (work == 'cu')
        {
			...
			document.getElementById('wr_1').value = document.getElementById('save_wr_1_' + comment_id).value;
