...
<aside id="stv">
 ...
 -->
		...
		//echo "select it_id, it_name , it_8 from {$g5['g5_shop_item_table']} where it_id = '$tv_it_id'";
            $rowx = sql_fetch(" select it_id, it_name , it_8 from {$g5['g5_shop_item_table']} where it_8 != '' and  it_id = '$tv_it_id' ");
            if(!$rowx['it_id'])
                continue;

            if ($tv_tot_count % $tv_div['img_length'] == 0) $k++;
			
			//$rowx2 = sql_fetch(" select it_id from {$g5['g5_shop_item_table']} where it_id = '$rowx[it_8]' ");
			
			$it_name = get_text($rowx['it_name']); 

            $img = get_it_image($rowx[it_8], $tv_div['img_width'], $tv_div['img_height'], '', '', $it_name);

            if ($tv_tot_count == 0) echo '<ul id="stv_ul">'.PHP_EOL;
            echo '<li class="stv_item c'.$k.'">'.PHP_EOL;
			echo '<a href="/shop/search.php?q='.$rowx[it_8].'&q_where=main">';
            echo $img;
			echo '</a>';
            //echo '<br>';
        ...
        $(function() {
            ...
            if (itemQty > itemShow)
            {
                $('#stv_btn').append('<button type="button" id="up"><img src="/img/main/quick/quick_prev.png"></button><button type="button" id="down"><img src="/img/main/quick/quick_next.png"></button>');
            }
            var Flag = 1; // 페이지
