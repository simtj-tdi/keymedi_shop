...
<table class="list_tb" style="width:95%">
	...
	<thead>
		<tr>
			...
			<th scope="col">상품명</th>
			<th scope="col" width="20%">규격</th>
			<th scope="col" width="5%">단위</th>
			<th scope="col" width="8%">제조사</th>
		</tr>
	</thead>
	<tbody>
<?php
for ($i=1; $row=sql_fetch_array($result); $i++) {
	...
	$rowd = sql_fetch("select it_name from g5_shop_item where it_id = '{$row['it_8']}' limit 1 ");
    /*
	...
	*/
	...
	?>
	<script>
		document.getElementById("itemview").src = "/shop/item.php?it_id=<?=$row['it_id']?>&no_frame=no&sub_item=Y";
	</script>
	<?
...
?> 
			...
			<tr class="this_tr" style="cursor:pointer;" onclick="parent.itemview.location.href='<?=$this->href?><?=$row['it_id']?>&no_frame=no&sub_item=Y';" >
				<td width="10%">
				<?
				if ($this->view_it_img) {
				...
				<td>
					<a href="<?=$this->href?><?=$row['it_id']?>&no_frame=no&sub_item=Y" target="itemview" class="sct_a">
				<?
				if ($this->view_it_name) {
					echo stripslashes($rowd['it_name']);
				}
				...
				<td width="30%"><?=stripslashes($rowd['it_5'])?></td>
				<td width="5%"><?=stripslashes($rowd['it_6'])?></td>
				<td width="15%"><?=cut_str(stripslashes($rowd['it_maker']),10)?></td>
			</tr>
...
<style>
.this_tr_n { border:2px solid #1180eb; border-left:4px solid #1180eb;}
</style>
<!-- } 상품진열 10 끝 -->